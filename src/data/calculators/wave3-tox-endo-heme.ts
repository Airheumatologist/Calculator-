...
export const wave3ToxEndoHemeCalcs: Calculator[] = [
  ...
  // ─── 5. Digoxin immune Fab ─────────────────────────────────────────────────
  {
    ...
    inputs: [
      selectInput('method', 'Estimation method', [
        { label: 'From steady-state serum level', value: 'level', description: 'vials = (level ng/mL × weight kg) / 100, rounded up. Prefer a post-distribution level (~6–8 h after last dose).' },
  ...
  // ─── 10. NMS criteria ──────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('exposure', 'Dopamine antagonist exposure OR withdrawal of dopamine agonist', 1, 'Typical onset days after starting or increasing an antipsychotic/antiemetic, or after abrupt withdrawal of a Parkinson dopamine agonist.'),
      yesNo('hyperthermia', 'Hyperthermia (typically ≥38 °C, often higher)', 1, 'Temperature ≥38.0 °C; often ≥38.5–40 °C in florid NMS.'),
  ...
  // ─── 15. Fagerström nicotine dependence ────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], undefined, 'How soon after you wake up do you smoke your first cigarette?'),
      selectInput('forbidden', 'Difficult to refrain in no-smoking places', [
        { label: 'No (0)', value: 0 },
        { label: 'Yes (1)', value: 1 },
      ], undefined, 'Do you find it difficult to refrain from smoking in places where it is forbidden (e.g. church, library, cinema, airplane)?'),
      ...
      ], undefined, 'Which cigarette would you hate most to give up?'),
      ...
      ], undefined, 'How many cigarettes per day do you smoke?'),
      selectInput('morningMore', 'Smoke more frequently during first hours after waking?', [
        { label: 'No (0)', value: 0 },
        { label: 'Yes (1)', value: 1 },
      ], undefined, 'Do you smoke more frequently during the first hours after waking than during the rest of the day?'),
      selectInput('illSmoke', 'Smoke when ill enough to be in bed most of the day?', [
        { label: 'No (0)', value: 0 },
        { label: 'Yes (1)', value: 1 },
      ], undefined, 'Do you smoke if you are so ill that you are in bed most of the day?'),
    ],
    calculate(values) {
      const score =
  ...
  // ─── 19. TI-RADS simplified ────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      selectInput('echogenicity', 'Echogenicity', [
        ...
        { label: 'Hyperechoic or isoechoic (1)', value: 1, description: 'Equal to or brighter than thyroid parenchyma. If echogenicity cannot be determined, assign 1.' },
        { label: 'Hypoechoic (2)', value: 2, description: 'Darker than thyroid parenchyma but not as dark as strap muscle' },
        { label: 'Very hypoechoic (3)', value: 3, description: 'Darker than strap muscle' },
      ], undefined, 'Compare hypo vs thyroid parenchyma; very hypo vs strap muscle. Undetermined echogenicity → assign 1.'),
      selectInput('shape', 'Shape', [
        { label: 'Wider-than-tall (0)', value: 0, description: 'AP diameter ≤ transverse diameter on the axial image' },
      ...
      selectInput('foci', 'Echogenic foci (choose highest / sum per ACR if multiple types)', [
        ...
        { label: 'Macrocalcifications (1)', value: 1, description: 'Coarse calcifications with acoustic shadowing' },
  ...
  // ─── 20. Bethesda thyroid ──────────────────────────────────────────────────
  {
    ...
    inputs: [
      selectInput('category', 'Bethesda category', [
        { label: 'I — Nondiagnostic / unsatisfactory', value: 1, description: 'Unsatisfactory sample; ROM ~5–10%. Repeat FNA with US guidance (often after ≥3 months unless high suspicion).' },
        { label: 'II — Benign', value: 2, description: 'Benign cytology; ROM ~0–3%. Clinical/sonographic follow-up; surgery not routine for cytology alone.' },
        { label: 'III — AUS / FLUS', value: 3, description: 'Atypia of undetermined significance; ROM ~10–30%. Repeat FNA, molecular testing, or diagnostic lobectomy.' },
        { label: 'IV — Follicular neoplasm / suspicious for FN', value: 4, description: 'Follicular neoplasm; ROM ~25–40%. Molecular testing or diagnostic surgery commonly considered.' },
        { label: 'V — Suspicious for malignancy', value: 5, description: 'Suspicious for malignancy; ROM ~50–75%. Surgical management typically recommended.' },
        { label: 'VI — Malignant', value: 6, description: 'Malignant cytology; ROM ~97–99%. Oncologic surgical planning.' },
      ], undefined, 'Enter the Bethesda category from the cytology report. Approximate risk of malignancy (ROM) is shown on each option — use local institutional ROM when available.'),
    ],
    calculate(values) {
  ...
  // ─── 21. Burch-Wartofsky thyroid storm ─────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], undefined, 'Use current core or reliable peripheral temperature. Bands are original BWPS °F cutoffs converted to °C (99 / 100 / 101 / 102 / 103 / ≥104 °F).'),
      selectInput('cns', 'CNS effects', [
        { label: 'Absent (0)', value: 0, description: 'Calm and oriented; no restlessness or agitation' },
        { label: 'Mild (agitation) (10)', value: 10, description: 'Restlessness, anxiety, or agitation; still oriented; no delirium, psychosis, or extreme lethargy' },
        { label: 'Moderate (delirium/psychosis/extreme lethargy) (20)', value: 20, description: 'Delirium (fluctuating inattention/disorientation), frank psychosis, or extreme lethargy that is still arousable' },
        { label: 'Severe (seizure/coma) (30)', value: 30, description: 'Seizure, or coma (unresponsive to verbal stimuli)' },
      ], undefined, 'Score the worst CNS finding only (do not add 10+20+30). Mild = agitation. Moderate = delirium, psychosis, or extreme lethargy. Severe = seizure or coma.'),
      selectInput('gi', 'GI-hepatic dysfunction', [
        { label: 'Absent (0)', value: 0, description: 'No diarrhea, nausea/vomiting, abdominal pain, or jaundice' },
        { label: 'Moderate (diarrhea/N/V/abdominal pain) (10)', value: 10, description: 'Diarrhea, nausea/vomiting, or abdominal pain attributed to thyrotoxicosis — no unexplained jaundice' },
        { label: 'Severe (unexplained jaundice) (20)', value: 20, description: 'Jaundice (scleral icterus or rising bilirubin) not explained by other hepatobiliary disease, obstruction, or hemolysis' },
      ], undefined, 'Score the worse GI-hepatic finding only. Moderate = GI symptoms. Severe = unexplained jaundice (do not add 10+20).'),
      ...
      ], undefined, 'Use current heart rate (including if already in AF). AF also scores separately on the items below.'),
      selectInput('chf', 'Congestive heart failure', [
        { label: 'Absent (0)', value: 0, description: 'No pedal edema, rales, or pulmonary edema' },
        { label: 'Mild (pedal edema) (5)', value: 5, description: 'Pretibial or pedal pitting edema without rales or clinical/radiographic pulmonary edema' },
        { label: 'Moderate (bibasilar rales) (10)', value: 10, description: 'Crackles at both bases on auscultation without frank pulmonary edema' },
        { label: 'Severe (pulmonary edema) (15)', value: 15, description: 'Frank pulmonary edema — rales throughout, orthopnea, or radiographic edema / need for NIPPV or intubation for flash edema' },
      ], undefined, 'Examine pretibial pits and auscultate both lung fields. Score the worst CHF finding only (do not sum 5+10+15).'),
      selectInput('afib', 'Atrial fibrillation', [
        { label: 'Absent (0)', value: 0, description: 'Sinus rhythm; no AF this episode' },
        { label: 'Present (10)', value: 10, description: 'Atrial fibrillation on ECG or telemetry this presentation (new or previously known)' },
      ], undefined, 'Score Present if AF is documented this episode, even if rate-controlled (tachycardia still scores on the HR item).'),
      selectInput('precipitant', 'Precipitant history', [
        { label: 'Negative (0)', value: 0, description: 'No identifiable precipitant' },
        { label: 'Positive (10)', value: 10, description: 'Infection, surgery, stopped/irregular thionamide, RAI, iodinated contrast, amiodarone, DKA, CVA/MI/PE, trauma, labor/delivery, or exogenous thyroid hormone' },
      ], undefined, 'Positive examples: infection, surgery, stopped/irregular thionamide, RAI, iodinated contrast, amiodarone, DKA, CVA/MI/PE, trauma, labor/delivery, exogenous thyroid hormone. If illness vs thyrotoxicosis is inseparable, score Positive (original favors diagnosing storm).'),
    ],
    calculate(values) {
      const score =
        num(values.temp, 0) +
