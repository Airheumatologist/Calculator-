...
export const wave7RheumClassCalcs: Calculator[] = [
  ...
  // ─── 2. 2019 EULAR/ACR SLE ───────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('ana', 'ANA ≥1:80 on HEp-2 (or equivalent) — entry criterion', null, 'Entry criterion: ANA ≥1:80 on HEp-2 cells or an equivalent assay, at any time. Count items only if SLE is the most likely explanation and only the highest item per domain; items need not be simultaneous.'),
  ...
  // ─── 10. 2022 ACR/EULAR TAK ──────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      selectInput('sex', 'Sex', TAK_SEX, undefined, 'Female sex scores 1 point in the Takayasu criteria; the entry conditions (age ≤60 with large-vessel vasculitis on imaging) must also be met before the total counts.'),
  ...
  // ─── 12. ASAS axial SpA ──────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('hlaB27', 'HLA-B27 positive', null, 'Yes if HLA-B27 is positive — it is the entry to the clinical arm, which then needs at least 2 further SpA features for classification.'),
  ...
  // ─── 13. ASAS peripheral SpA ─────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('hlaB27', 'HLA-B27 positive', null, 'Yes if HLA-B27 is positive; on its own it is one group-A feature, and 2 group-B features are the alternative route to classification.'),
      yesNo('uveitis', 'Uveitis', null,
        'Typically acute anterior uveitis diagnosed by an ophthalmologist.'),
  ...
  // ─── 17. 2023 ACR/EULAR APS ──────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      selectInput('aclIgm', 'IgM anticardiolipin', APS_ACL_IGM, undefined, 'One of the two solid-phase assays. The 2023 criteria combine anticardiolipin and anti-β2GPI results into cluster weights rather than adding each assay separately.'),
      ...
      selectInput('b2Igm', 'IgM anti-β2-glycoprotein I', APS_B2_IGM, undefined, 'The other solid-phase assay; it enters the same cluster weight as anticardiolipin, so record it as positive or negative rather than as an extra point.'),
    ],
    calculate(values) {
