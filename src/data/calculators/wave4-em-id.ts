...
export const wave4EmIdCalcs: Calculator[] = [
...

  {
    ...
    inputs: [
      yesNo('cancer', 'Active cancer', 2, 'Diagnosed within 6 months before the VTE (exclude BCC/SCC skin), recently recurrent/progressive, or anticancer treatment within 6 months.'),
...

  {
    ...
    inputs: [
      yesNo('painPop', 'Pain out of proportion to skin findings', 2, 'Pain severe relative to the visible cellulitis, often beyond the erythema border — a key early NSTI clue.'),
      yesNo('rapidSpread', 'Rapid progression of erythema/edema', 2, 'Visible spread over hours (not days) on serial exam or history.'),
      yesNo('tenseEdema', 'Tense or woody edema beyond visible erythema', 2, 'Wood-hard induration that does not pit like typical cellulitis, extending past the red border.'),
      yesNo('crepitus', 'Crepitus or gas on exam/imaging', 3, 'Palpable soft-tissue gas or radiographic/CT gas in fascia/soft tissue (hard sign).'),
...

  {
    ...
    inputs: [
      yesNo('fever', 'Fever in past 24 hours', 1, 'Documented or patient-reported fever during the previous 24 hours (no specific °C cutoff).'),
      yesNo('purulence', 'Purulence (pus on tonsils)', 1, 'Pus visible on the tonsils — not nasal discharge.'),
      yesNo('attend3', 'Attend rapidly (illness ≤3 days)', 1, 'Symptom onset ≤3 days before this attendance.'),
      yesNo('inflamed', 'Severely inflamed tonsils', 1, 'Marked tonsillar erythema and swelling (beyond everyday pharyngitis redness) — original FeverPAIN “severely inflamed tonsils”.'),
...

  {
    ...
    inputs: [
      yesNo('unilateral', 'Unilateral tonsillar swelling / fullness', 2, 'Peritonsillar fullness on one side (not symmetric kissing tonsils).'),
      yesNo('uvula', 'Uvular deviation away from the lesion', 2, 'Uvula pushed away from the swollen peritonsillar side.'),
      yesNo('trismus', 'Trismus (limited mouth opening)', 2, 'Cannot insert 3 finger-breadths between incisors (or interincisal distance typically <30–35 mm) from pterygoid spasm.'),
      yesNo('hotPotato', 'Hot-potato / muffled voice', 1, 'Muffled voice as if speaking with a hot potato in the mouth.'),
      yesNo('drooling', 'Drooling or inability to swallow secretions', 2, 'Cannot handle saliva — airway-risk cue.'),
      yesNo('earPain', 'Referred otalgia', 1, 'Ipsilateral ear pain without primary otitis.'),
      yesNo('feverTox', 'Fever or systemic toxicity', 1, 'Temperature ≥38.0 °C or ill appearance.'),
      yesNo('fluctuant', 'Palpable fluctuance of soft palate (if examined)', 2, 'Palpable fluid collection in the soft palate — only if the patient can open enough to examine.'),
    ],
    calculate(values) {
      const score =
...

  {
    ...
    inputs: [
      yesNo('severeOdynophagia', 'Severe odynophagia / inability to swallow secretions', 2, 'Pain on swallowing out of proportion to oropharyngeal exam, or cannot handle saliva.'),
      yesNo('voiceChange', 'Muffled / hot-potato voice', 1, 'Muffled phonation without primary tonsillar abscess findings.'),
      yesNo('tripod', 'Tripod / sniffing position', 2, 'Sitting, leaning forward, neck extended, mouth open — classic airway-preserving posture.'),
      yesNo('stridor', 'Stridor or noisy breathing', 3, 'High-pitched inspiratory noise at rest (not only with crying). Airway-threatening.'),
...

  {
    ...
    inputs: [
      yesNo('ageYoung', 'Age typically <5 years (classic demography)', 1, 'Classic RPA is most common under age 5 (retropharyngeal nodes involute later).'),
      yesNo('fever', 'Fever', 1, 'Temperature typically ≥38.0 °C.'),
      yesNo('neckStiff', 'Neck stiffness or limited neck extension', 2, 'Limited extension (child refuses to look up) is the teaching discriminator vs meningitis flexion rigidity.'),
      yesNo('torticollis', 'Torticollis / preferential head position', 2, 'Head tilted/rotated to one side; child resists passive correction.'),
      yesNo('drooling', 'Drooling or dysphagia', 2, 'Cannot handle secretions or painful swallowing.'),
...

  {
    ...
    inputs: [
      selectInput('agePts', 'Age', [
        { label: '<60 years (0)', value: 0, description: 'Age <60 years at CDI diagnosis (0 points).' },
        { label: '60–79 years (1)', value: 1, description: 'Age 60–79 years (1 point).' },
        { label: '≥80 years (2)', value: 2, description: 'Age ≥80 years (2 points).' },
      ], undefined, 'Age at CDI diagnosis.'),
      yesNo('systemicAbx', 'Systemic antibiotics during CDI therapy (not anti-CDI agent)', 2, 'Concurrent systemic antibiotics that are not the anti-CDI agent (vancomycin/fidaxomicin/metronidazole). Stewardship target — stop nonessential agents.'),
      selectInput('wbcPts', 'Leukocyte count (×10³/µL)', [
        { label: '<16 (0)', value: 0, description: 'WBC <16 ×10³/µL (0 points).' },
        { label: '16–25 (1)', value: 1, description: 'WBC 16–25 ×10³/µL (1 point).' },
        { label: '>25 (2)', value: 2, description: 'WBC >25 ×10³/µL (2 points).' },
      ], undefined, 'Peak leukocyte count during the CDI episode (×10³/µL).'),
      selectInput('albPts', 'Albumin (g/dL)', [
        { label: '>3.5 (0)', value: 0, description: '>3.5 g/dL (≈ >35 g/L).' },
...

  {
    ...
    inputs: [
      ...
      yesNo('thunderclap', 'Thunderclap headache (instantly peaking pain)', 1, 'Pain peaks instantly (seconds), not over minutes. Eligibility: alert ≥15 y, new severe atraumatic headache peaking ≤1 h.'),
      yesNo('limitedFlexion', 'Limited neck flexion on examination', 1, 'Cannot touch chin to chest, or cannot raise the head 8 cm off the bed if supine (exam finding — distinct from reported neck pain). Apply only to alert ≥15 y with new severe atraumatic headache peaking ≤1 h; exclude new deficit, prior aneurysm/SAH/tumor, ≥3 similar headaches over ≥6 months.'),
    ],
    calculate(values) {
...

  {
    ...
    inputs: [
      yesNo('posterior', 'Isolated posterior MI (anterior STD + tall R / posterior leads STE)', 1, 'V1–V3 ST depression ± tall R; confirm with V7–V9 STE ≥0.5 mm.'),
      yesNo('deWinter', 'de Winter T-wave pattern', 1, '1–3 mm upsloping J-point ST depression in V1–V6 into tall peaked T waves ± aVR 0.5–1 mm STE.'),
      yesNo('wellens', 'Wellens syndrome (pain-free biphasic/deep inverted precordial T)', 1, 'Pain-free biphasic (Type A) or deep inverted (Type B) T waves in V2–V3, STE <1 mm, preserved R waves — not a stress-test candidate.'),
      yesNo('sgarbossa', 'Sgarbossa / Smith-modified positive in LBBB or paced rhythm', 1, 'Smith-modified: concordant STE ≥1 mm, concordant STD ≥1 mm in V1–V3, or ST/S ≤ −0.25.'),
