...
export const missingHemeIdNephroCalcs: Calculator[] = [
  ...
  },
  {
    ...
    inputs: [
      yesNo('plt', 'Platelet count < 30 × 10⁹/L', 1, 'Admission/presentation platelet count <30 × 10⁹/L (not after transfusion).', true),
      yesNo('hemolysis', 'Hemolysis evidence (indirect bili >2 mg/dL, retic >2.5%, or undetectable haptoglobin)', 1, 'Any one: indirect bilirubin >2 mg/dL, reticulocytes >2.5%, or undetectable haptoglobin. Schistocytes support TMA but are not a PLASMIC point.', true),
      yesNo('noCancer', 'No active cancer', 1, 'Yes = NO active cancer (solid or hematologic). Active cancer scores 0 on this item.', true),
      yesNo('noTransplant', 'No solid-organ or stem-cell transplant', 1, 'Yes = NO solid-organ or HSCT history. Prior transplant scores 0 on this item.', true),
      yesNo('mcv', 'MCV < 90 fL', 1, 'Mean corpuscular volume <90 fL (surrogate for microangiopathic fragmentation).', true),
      yesNo('inr', 'INR < 1.5', 1, 'INR <1.5 (near-normal coagulation; higher INR argues against TTP toward DIC/liver).', true),
      yesNo('cr', 'Creatinine < 2.0 mg/dL', 1, 'Creatinine <2.0 mg/dL (≈177 µmol/L). Higher Cr is more aHUS/other TMA than TTP.', false),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      ], 1, 'Current platelet count (×10⁹/L). Requires an underlying DIC-associated condition to interpret as overt DIC.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      ], 2, 'Primary site at start of systemic therapy. Very high = stomach or pancreas (2); high = lung, lymphoma, gyn, bladder, testicular (1).'),
      yesNo('plt', 'Pre-chemo platelet count ≥ 350 × 10⁹/L', 1, 'Platelet count ≥350 × 10⁹/L before this cycle of chemotherapy.', true),
      yesNo('hb', 'Hemoglobin < 10 g/dL and/or using ESA', 1, 'Hb <10 g/dL and/or currently receiving an erythropoiesis-stimulating agent.', true),
      yesNo('wbc', 'Pre-chemo leukocyte count > 11 × 10⁹/L', 1, 'Pre-chemo WBC >11 × 10⁹/L.', false),
      yesNo('bmi', 'BMI ≥ 35 kg/m²', 1, 'Body mass index 35 kg/m² or higher at the start of systemic therapy; scores 1 point in the Khorana model.', false),
    ],
    calculate(values) {
      const score =
        num(values.site) +
  ...
  },
  {
    ...
    inputs: [
      ...
      yesNo('noHypotension', 'No hypotension (SBP > 90 mmHg)', 5, 'Yes = SBP >90 mmHg (no hypotension). Hypotension scores 0 on this item and usually precludes outpatient care.', true),
      yesNo('noCopd', 'No COPD', 4, 'Yes = no COPD. Known COPD scores 0 on this item.', true),
      yesNo('solidOrNoFungal', 'Solid tumor OR hematologic malignancy with no prior fungal infection', 4, 'Yes if solid tumor, OR hematologic malignancy without prior invasive fungal infection. Prior fungal infection in heme malignancy scores 0.', true),
      yesNo('noDehydration', 'No dehydration requiring parenteral fluids', 3, 'Yes = not dehydrated enough to need IV fluids. Dehydration requiring parenteral fluids scores 0.', true),
      yesNo('outpatient', 'Outpatient status at fever onset', 3, 'Yes = fever started while an outpatient. Inpatient fever onset scores 0.', true),
      yesNo('age', 'Age < 60 years', 2, 'Yes = age <60. Age ≥60 scores 0 on this item.', true),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('fever', 'Temperature > 38°C or history of fever', 1, 'T >38.0 °C now or a clear history of fever with this illness.', true),
      yesNo('noCough', 'Absence of cough', 1, 'Yes = no cough. Cough present scores 0 (more viral).', true),
      yesNo('tender', 'Tender anterior cervical nodes', 1, 'Tender anterior cervical lymphadenopathy (not posterior-only).', true),
      yesNo('exudate', 'Tonsillar swelling or exudate', 1, 'Tonsillar swelling and/or exudate on exam.', false),
      ...
      ], 1, 'Age band scores: 3–14 years +1, 15–44 years 0, ≥45 years −1; under 3 years scores 0 because GAS pharyngitis and rheumatic fever are rare and testing is not routinely recommended.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('fever', 'Fever lasting ≥ 5 days (or fever present and KD strongly suspected)', 0, 'Classic KD: fever ≥5 days. AHA allows treating earlier if ≥4 principal features and KD is strongly suspected. Incomplete KD uses lab/echo supplements.', true),
      yesNo('conjunctivitis', 'Bilateral bulbar conjunctival injection (nonexudative)', 1, 'Bilateral, nonexudative, often spares the limbus (perilimbal sparing). Not bacterial conjunctivitis with discharge.', true),
      yesNo('oral', 'Oral mucosal changes (strawberry tongue, red cracked lips, injected pharynx)', 1, 'Strawberry tongue, erythematous cracked lips, and/or diffuse injected oropharynx — not isolated exudative tonsillitis.', true),
      yesNo('extremity', 'Extremity changes (erythema/edema of hands/feet or periungual peeling)', 1, 'Acute: erythema or firm edema of palms/soles. Convalescent: periungual desquamation (usually week 2–3).', true),
      yesNo('rash', 'Polymorphous rash', 1, 'Maculopapular, diffuse erythroderma, or erythema multiforme-like. Not typically vesicular or bullous. May include perineal accentuation.', true),
      yesNo('nodes', 'Cervical lymphadenopathy (≥1.5 cm, usually unilateral)', 1, '≥1.5 cm, usually unilateral anterior cervical. Least common principal criterion.', false),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('well', 'Infant appears well (nontoxic)', 1, 'Nontoxic = alert, well-perfused, age-appropriate interaction. Historically well-appearing febrile infants ≤60 days with T ≥38°C; not a substitute for AAP 2021/PECARN.', true),
      yesNo('previouslyHealthy', 'Previously healthy term infant (no antibiotics, no underlying disease, not prolonged neonatal stay)', 1, 'Term ≥37 wks; no perinatal or current antibiotics; not hospitalized longer than the mother; no unexplained hyperbilirubinemia; no underlying illness.', true),
      yesNo('noFocus', 'No ear, soft tissue, bone, or skin infection on exam', 1, 'No otitis, cellulitis, osteomyelitis, or other focal bacterial infection on exam.', true),
      yesNo('wbcOk', 'WBC 5,000–15,000/mm³', 1, 'WBC 5–15 × 10³/µL.', true),
      yesNo('bandsOk', 'Absolute band count ≤ 1,500/mm³', 1, 'Absolute band count ≤1,500/mm³ (1.5 × 10⁹/L).', true),
      yesNo('uaOk', 'UA ≤ 10 WBC/hpf', 1, 'Spun UA ≤10 WBC/hpf (not nitrites/LE as a substitute in classic Rochester).', true),
      yesNo('stoolOk', 'If diarrhea: stool ≤ 5 WBC/hpf (or no diarrhea)', 1, 'If diarrhea is present, stool ≤5 WBC/hpf. If no diarrhea, score Yes.', true),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      ], 'M', 'Male sex is a published IMPROVE-Bleed item worth 1 point; female sex scores 0.'),
      numberInput('gfr', 'GFR', { unit: 'mL/min/1.73 m²', min: 5, max: 120, exampleValue: 70, helpText: '≥60: 0; 30–59: 1; <30: 2.5' }),
      yesNo('cancer', 'Active cancer (within 6 months)', 2, 'Cancer diagnosed or treated within 6 months, or metastatic disease.', true),
      yesNo('rheumatic', 'Rheumatic disease', 2, 'Active rheumatic/autoimmune disease (e.g. RA, SLE) as in IMPROVE.', false),
      yesNo('cvc', 'Central venous catheter', 2, 'Central line in situ (PICC, port, internal jugular/subclavian CVC).', true),
      yesNo('icu', 'ICU/CCU admission', 2.5, 'Admitted to ICU or CCU during this hospitalization.', true),
      yesNo('liver', 'Hepatic failure (INR >1.5)', 2.5, 'INR >1.5 attributed to hepatic failure (not isolated warfarin).', false),
      numberInput('plt', 'Platelet count', { unit: '×10⁹/L', min: 5, max: 800, exampleValue: 200, helpText: '<50: +4' }),
      yesNo('recentBleed', 'Bleeding in the 3 months before admission', 4, 'Any clinically significant bleed in the 3 months before this admission.', true),
      yesNo('ulcer', 'Active gastroduodenal ulcer', 4.5, 'Active gastroduodenal ulcer (not remote PUD history alone).', false),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      numberInput('weight', 'Body weight', { unit: 'kg', unitKind: 'weight', min: 2, max: 300, exampleValue: 70, helpText: 'Current body weight; choose kg or lb and the app converts. The estimate scales linearly with weight.' }),
      ...
      ], 0.5, 'Adrogué–Madias TBW fraction. Use the closest demographic; actual TBW varies with obesity and cachexia.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      yesNo('infection', 'Suspected or documented infection', 0, 'Yes if infection is suspected or documented. Sepsis-3 requires infection plus ΔSOFA ≥2.', true),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('hepaticRenal', 'Hepatic or renal disease', 1, 'Hepatic = cirrhosis, AST or ALP ≥2× ULN, or albumin <3.6 g/dL. Renal = CrCl <30 mL/min.', true),
      yesNo('etoh', 'Ethanol abuse', 1, 'Abuse history or recent alcohol-related admission.', false),
      yesNo('malignancy', 'Malignancy', 1, 'Recent or metastatic cancer.', true),
      yesNo('older', 'Older age (> 75 years)', 1, 'Age above 75 years at admission; scores 1 point in HEMORR₂HAGES.', true),
      yesNo('reducedPlt', 'Reduced platelet count or function (including aspirin)', 1, 'Plt <75 × 10⁹/L or scheduled antiplatelet/NSAID.', false),
      yesNo('rebleeding', 'Rebleeding — prior major bleed (2 points)', 2, 'Prior hospitalization for bleeding.', false),
      yesNo('htn', 'Hypertension (uncontrolled)', 1, 'Most recent SBP >160 mmHg.', true),
      yesNo('anemia', 'Anemia', 1, 'Hct <30% or Hb <10 g/dL.', true),
      yesNo('genetic', 'Genetic factors (e.g., CYP2C9 variant) if known', 1, 'CYP2C9*2 and/or *3 if known; skip if unknown (do not guess).', false),
      yesNo('falls', 'Excessive fall risk', 1, 'Dementia, Parkinson, schizophrenia, or repeated-fall condition.', false),
      yesNo('stroke', 'Stroke history', 1, 'Any prior ischemic stroke or intracranial hemorrhage; scores 1 point. Asymptomatic small-vessel disease on imaging alone is not the intent of this item.', false),
    ],
    calculate(values) {
      const score =
  ...
  },
  {
    ...
    inputs: [
      yesNo('older', 'Older age (≥ 75 years) — 1 point', 1, 'Age ≥75 years.', true),
      yesNo('anemia', 'Reduced hemoglobin / anemia (or Hct <40 men / <36 women) — 2 points', 2, 'Anemia or Hct <40% (men) / <36% (women) as in ORBIT-AF.', true),
      yesNo('bleed', 'Bleeding history — 2 points', 2, 'Any prior clinically significant / major bleed (GI, ICH, or other hospitalization for bleeding).', false),
      yesNo('renal', 'Insufficient kidney function (eGFR < 60 mL/min/1.73m²) — 1 point', 1, 'eGFR <60 mL/min/1.73 m².', true),
      yesNo('antiplt', 'Treatment with antiplatelet — 1 point', 1, 'Aspirin, P2Y12 inhibitor, or other antiplatelet at the time of scoring.', true),
    ],
    calculate(values) {
      const score =
        (bool(values.older) ? 1 : 0) +
