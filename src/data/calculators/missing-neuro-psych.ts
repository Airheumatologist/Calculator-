...
export const missingNeuroPsychCalcs: Calculator[] = [
  ...
  },
  {
    ...
    calculate(values) {
      const mode = String(values.mode ?? 'checkboxes');
      let score: number;
      let regions: number;

      if (mode === 'direct' || (values.score !== undefined && values.mode === undefined && values.reg_c === undefined && values.regionsLost === undefined)) {
        ...
        regions =
          (bool(values.reg_c) ? 1 : 0) +
          (bool(values.reg_l) ? 1 : 0) +
          (bool(values.reg_ic) ? 1 : 0) +
          (bool(values.reg_i) ? 1 : 0) +
          (bool(values.reg_m1) ? 1 : 0) +
          (bool(values.reg_m2) ? 1 : 0) +
          (bool(values.reg_m3) ? 1 : 0) +
          (bool(values.reg_m4) ? 1 : 0) +
          (bool(values.reg_m5) ? 1 : 0) +
          (bool(values.reg_m6) ? 1 : 0);
        score = 10 - regions;
      }
      ...
      return {
        ...
        ...r,
        details: [
          { label: 'Entry mode', value: mode === 'checkboxes' ? '10 anatomical checkboxes' : mode === 'regions' ? 'Count regions (10 − n)' : 'Direct ASPECTS total' },
    ...
    nextSteps: [
      {
        condition: 'ASPECTS ≥6 with anterior circulation LVO in window',
        actions: ['Expedite endovascular thrombectomy (EVT)', 'Reperfusion pathway', 'Monitor vitals and neuro checks'],
      },
      {
        condition: 'ASPECTS 0–5 (large core)',
        actions: [
          'Emerging evidence supports thrombectomy in selected large core infarcts',
          'Discuss risks/benefits with neurointerventionalist and family',
          'Perfusion imaging / clinical severity context',
        ],
      },
    ],
    pearls: [
      'ASPECTS applies only to the middle cerebral artery territory (not ACA, PCA, or posterior fossa).',
      'Early ischemic changes include parenchymal hypoattenuation and loss of gray–white differentiation.',
      'Chronic infarcts (sharp margins, encephalomalacia) should not be scored as acute ASPECTS loss.',
    ],
  },
  {
    id: 'mrs',
