...
export const wave2PulmIdCalcs: Calculator[] = [
  {
    ...
    inputs: [
      yesNo('shock', 'Septic shock requiring vasopressors (major)', 1, 'Infection-related shock needing vasopressors to maintain BP — this single major criterion defines severe CAP.', false),
      yesNo('vent', 'Respiratory failure requiring mechanical ventilation (major)', 1, 'Invasive mechanical ventilation for respiratory failure (the other major criterion).', true),
      yesNo('rr', 'Respiratory rate ≥ 30/min (minor)', 1, 'Respiratory rate 30/min or faster (minor criterion, 1 point). Three or more minor criteria (or one major) support ICU-level care.', true),
      yesNo('pf', 'PaO₂/FiO₂ ≤ 250 (minor)', 1, 'PaO₂/FiO₂ ≤250 mmHg (PaO₂ in mmHg ÷ FiO₂ as a fraction).', true),
      yesNo('multilobar', 'Multilobar infiltrates (minor)', 1, 'Infiltrates in ≥2 lobes on CXR or CT.', false),
      yesNo('confusion', 'Confusion / disorientation (minor)', 1, 'New disorientation to person/place/time (or equivalent). If baseline dementia, score only acute change.', false),
      yesNo('urea', 'Uremia — BUN ≥ 20 mg/dL (minor)', 1, 'BUN 20 mg/dL or higher (urea ≈7.1 mmol/L) — the uremia minor criterion, 1 point.', true),
      yesNo('leukopenia', 'Leukopenia — WBC < 4,000/µL (minor)', 1, 'WBC below 4,000/µL (4.0 ×10⁹/L) from leukocytosis-driven infection, not a chronic or drug-related baseline — 1 point.', false),
      yesNo('thrombocytopenia', 'Thrombocytopenia — platelets < 100,000/µL (minor)', 1, 'Platelets below 100,000/µL (100 ×10⁹/L) — 1 point. Exclude chronic ITP or dilutional causes when attributing it to the pneumonia.', false),
      yesNo('hypothermia', 'Hypothermia — core temp < 36°C (minor)', 1, 'Core temperature below 36°C — 1 point. Use a core/rectal or esophageal reading; an axillary value is unreliable for this criterion.', false),
      yesNo('hypotension', 'Hypotension requiring aggressive fluid resuscitation (minor)', 1, 'SBP <90 mmHg or MAP <65 that requires aggressive fluids to restore BP. If vasopressors are needed, score the major septic-shock criterion instead.', false),
    ],
    calculate(values) {
      const major =
  ...
  },
  {
    ...
    inputs: [
      yesNo('ph', 'Arterial pH < 7.30 (+13)', 13, 'Arterial pH below 7.30 on the ABG (+13 points, the highest-weighted SCAP item). Venous or capillary pH is not a substitute.', false),
      yesNo('sbp', 'SBP < 90 mmHg (+11)', 11, 'Systolic BP below 90 mmHg (+11 points) — use the lowest value before fluids are given.', false),
      yesNo('rr', 'Respiratory rate > 30/min (+9)', 9, 'Respiratory rate above 30/min (+9 points); count the rate documented with the same vitals set as the other fields.', true),
      yesNo('pao2', 'PaO₂ < 54 mmHg or PaO₂/FiO₂ < 250 (+6)', 6, 'PaO₂ below 54 mmHg, or PaO₂/FiO₂ below 250 (+6 points). Either threshold qualifies — do not double-count.', true),
      yesNo('bun', 'BUN > 30 mg/dL (+5)', 5, 'BUN above 30 mg/dL (+5 points; about 10.7 mmol/L urea).', true),
      yesNo('ams', 'Altered mental status — new disorientation to person/place/time or GCS <15 (+5)', 5, 'If baseline dementia, score only acute change. España AMS is new confusion, not chronic cognitive impairment without change.', false),
      yesNo('age80', 'Age ≥ 80 years (+5)', 5, 'Age 80 years or older (+5 points). Younger ages score 0 — there is no graded age term.', false),
      yesNo('multilobar', 'Multilobar or bilateral infiltrate (+5)', 5, 'Multilobar or bilateral infiltrate on chest imaging (+5 points); a single-lobe pattern scores 0.', false),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('temp', 'Temperature ≤ 37.8°C (100°F)', 1, 'Temperature 37.8°C (100°F) or lower — 1 of 7 core stability criteria. Defervescence means a sustained value, not a single afebrile reading.', true),
      yesNo('hr', 'Heart rate ≤ 100 beats/min', 1, 'Heart rate 100 beats/min or lower (off chronotropes, unless the patient is beta-blocked and normally tachycardic).', true),
      yesNo('rr', 'Respiratory rate ≤ 24 breaths/min', 1, 'Respiratory rate 24 breaths/min or lower while at rest.', true),
      yesNo('sbp', 'Systolic BP ≥ 90 mmHg', 1, 'Systolic BP 90 mmHg or higher and without vasopressors.', true),
      yesNo('o2', 'O₂ sat ≥ 90% on room air (or at baseline O₂ requirement)', 1, 'Oxygen saturation 90% or above on room air, or on a requirement at the patient\'s chronic baseline (stable home O₂ counts).', true),
      yesNo('mental', 'Normal mental status (or return to baseline)', 1, 'Normal mental status, or a return to the documented baseline (chronic dementia that is unchanged still counts as met).', true),
      yesNo('oral', 'Able to take oral medications / functioning GI tract', 1, 'Able to swallow and absorb oral medication — no ileus, significant vomiting, or NPO order.', true),
      yesNo('stable24', 'Criteria sustained ~24 hours (optional clinical check)', 0, 'Optional confirmation that criteria have held for about 24 hours; a single snapshot that meets criteria is a weaker basis for switching to oral therapy.', true),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('multilobar', 'Multilobar infiltrate (+5)', 5, 'Multilobar infiltrate on imaging (+5 points, the largest single item).', true),
      yesNo('lymphopenia', 'Lymphopenia ≤ 0.8 × 10⁹/L (+4)', 4, 'Absolute lymphocyte count 0.8 ×10⁹/L or lower (+4 points); use the count, not the percentage.', true),
      yesNo('bacterial', 'Bacterial coinfection (+4)', 4, 'Yes if proven (blood or respiratory culture with a bacterial pathogen) or a typical bacterial presentation with supporting labs. Do not tick for viral pneumonia alone.', false),
      ...
      yesNo('htn', 'Hypertension (+2)', 2, 'History of hypertension (+2 points) — diagnosis or current antihypertensive therapy.', false),
      yesNo('age60', 'Age ≥ 60 years (+2)', 2, 'Age 60 years or older (+2 points). Total ≥12 marks the higher-mortality band in the derivation cohorts.', true),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('fever', 'History of fever or measured temp ≥38°C', 1, 'History of fever or a measured temperature 38°C or higher (+1 point); a subjective history qualifies.', true),
      yesNo('noCough', 'Absence of cough', 1, 'Absence of cough (+1 point) — the item is scored when cough is absent, so answer Yes for a dry throat with no cough.', true),
      yesNo('tender', 'Tender anterior cervical lymphadenopathy', 1, 'Palpate the anterior cervical chain; Yes if nodes are enlarged and tender (not posterior-chain or nontender).', true),
      yesNo('exudate', 'Tonsillar exudate or swelling', 1, 'Exudate or swelling of the tonsils on oropharynx exam (not isolated pharyngeal erythema).', false),
      ...
      ], 1, 'McIsaac age adjustment: 3–14 years +1, 15–44 years 0, 45 years or older −1; under 3 years scores 0 and GAS testing is rarely indicated.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      selectInput('age', 'Age', [
        ...
        { label: '≥ 80 (7)', value: 7 },
      ], 4, 'Age at admission band: <50 years 0, 50–59 2, 60–69 4, 70–79 6, 80+ 7 — the dominant predictor of mortality in the model.'),
      selectInput('sex', 'Sex at birth', [
        { label: 'Female (0)', value: 0 },
        { label: 'Male (1)', value: 1 },
      ], 1, 'Sex at birth: male adds 1 point, female 0, per the ISARIC 4C coefficients.'),
      selectInput('comorbid', 'Number of comorbidities', [
        { label: '0 (0)', value: 0 },
        { label: '1 (1)', value: 1 },
        { label: '≥ 2 (2)', value: 2 },
      ], 1, 'Count chronic cardiac; chronic respiratory excluding asthma; chronic renal (eGFR ≤30); liver disease; dementia; chronic neurological; connective tissue; diabetes; HIV/AIDS; malignancy; clinician-defined obesity. Do not count asthma alone.'),
      selectInput('rr', 'Respiratory rate (breaths/min)', [
        { label: '< 20 (0)', value: 0 },
        { label: '20–29 (1)', value: 1 },
        { label: '≥ 30 (2)', value: 2 },
      ], 1, 'Admission respiratory rate (breaths/min).'),
      selectInput('spo2', 'SpO₂ on room air', [
        { label: '≥ 92% (0)', value: 0 },
        { label: '< 92% (2)', value: 2 },
      ], 0, 'Admission SpO₂ on room air when feasible (ISARIC 4C points assume room air).'),
      selectInput('gcs', 'GCS', [
        { label: '15 (0)', value: 0, description: 'GCS 15' },
        { label: '< 15 (2)', value: 2, description: 'GCS <15 from any cause, including new confusion' },
      ], 0, 'Admission GCS. Score 2 if GCS <15 (including new confusion). If sedated, estimate pre-sedation GCS.'),
      ...
      ], 1, 'ISARIC 4C: urea <7 mmol/L = 0 points; urea exactly 7 mmol/L scores 1.'),
      selectInput('crp', 'CRP (mg/L)', [
        { label: '< 50 (0)', value: 0 },
        { label: '50–99 (1)', value: 1 },
        { label: '≥ 100 (2)', value: 2 },
      ], 2, 'C-reactive protein in mg/L: <50 scores 0, 50–99 scores 1, 100 or above scores 2. Use the admission or first-24-hour value.'),
    ],
    calculate(values) {
      const score =
        num(values.age) +
  ...
  },
  {
    ...
    inputs: [
      selectInput('cxr', 'Chest radiograph (alveolar consolidation)', [
        { label: 'No consolidation (0)', value: 0, description: 'No alveolar consolidation in any quadrant' },
        { label: '1 quadrant (1)', value: 1, description: 'Alveolar consolidation confined to 1 of 4 quadrants' },
        { label: '2 quadrants (2)', value: 2, description: 'Alveolar consolidation in 2 quadrants' },
        { label: '3 quadrants (3)', value: 3, description: 'Alveolar consolidation in 3 quadrants' },
        { label: '4 quadrants (4)', value: 4, description: 'Alveolar consolidation in all 4 quadrants' },
      ], 2, 'Divide the CXR into 4 quadrants (right/left × upper/lower, split at the hilum). Count quadrants with alveolar consolidation — not interstitial markings alone.'),
      selectInput('hypox', 'Hypoxemia (PaO₂/FiO₂)', [
        { label: '≥ 300 (0)', value: 0, description: 'PaO₂/FiO₂ ≥300 mmHg' },
        { label: '225–299 (1)', value: 1, description: 'PaO₂/FiO₂ 225–299 mmHg' },
        { label: '175–224 (2)', value: 2, description: 'PaO₂/FiO₂ 175–224 mmHg' },
        { label: '100–174 (3)', value: 3, description: 'PaO₂/FiO₂ 100–174 mmHg' },
        { label: '< 100 (4)', value: 4, description: 'PaO₂/FiO₂ <100 mmHg' },
      ], 1, 'PaO₂/FiO₂ in mmHg (PaO₂ in mmHg ÷ FiO₂ as a fraction).'),
      selectInput('peep', 'PEEP (cm H₂O)', [
        { label: '≤ 5 (0)', value: 0, description: 'PEEP ≤5 cm H₂O' },
        { label: '6–8 (1)', value: 1, description: 'PEEP 6–8 cm H₂O' },
        { label: '9–11 (2)', value: 2, description: 'PEEP 9–11 cm H₂O' },
        { label: '12–14 (3)', value: 3, description: 'PEEP 12–14 cm H₂O' },
        { label: '≥ 15 (4)', value: 4, description: 'PEEP ≥15 cm H₂O' },
      ], 1, 'Set PEEP at the time of scoring (cm H₂O). If not ventilated, this component is typically 0.'),
      selectInput('compliance', 'Respiratory system compliance (mL/cm H₂O)', [
        { label: '≥ 80 (0)', value: 0, description: 'Crs ≥80 mL/cm H₂O' },
        { label: '60–79 (1)', value: 1, description: 'Crs 60–79 mL/cm H₂O' },
        { label: '40–59 (2)', value: 2, description: 'Crs 40–59 mL/cm H₂O' },
        { label: '20–39 (3)', value: 3, description: 'Crs 20–39 mL/cm H₂O' },
        { label: '≤ 19 (4)', value: 4, description: 'Crs ≤19 mL/cm H₂O' },
      ], 2, 'Crs = VT (mL) / (Pplat − PEEP) in mL/cm H₂O. Use static (inspiratory-hold) compliance when available.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      numberInput('map', 'Mean airway pressure (MAP)', { unit: 'cm H₂O', min: 0, max: 50, step: 0.5, exampleValue: 15, helpText: 'Mean airway pressure in cmH₂O from the ventilator (MAP shown by the respiratory therapist\'s vent data, not mean arterial pressure).' }),
      numberInput('pao2', 'PaO₂', { unit: 'mmHg', min: 20, max: 600, step: 1, exampleValue: 60, helpText: 'Arterial PaO₂ in mmHg from the same ventilator settings used for the MAP; OI = FiO₂ × MAP × 100 ÷ PaO₂.' }),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      numberInput('paco2', 'PaCO₂', { unit: 'mmHg', min: 10, max: 120, step: 0.1, exampleValue: 40, helpText: 'Arterial PaCO₂ in mmHg from the same steady state as the mixed-expired CO₂, which must be entered in the companion field. VD/VT = (PaCO₂ − PECO₂) ÷ PaCO₂.' }),
  ...
  },
  {
    ...
    inputs: [
      numberInput('age', 'Age', { unit: 'years', min: 18, max: 110, step: 1, exampleValue: 60, helpText: 'Age in years, used as 100 − 0.3 × age; the expected PaO₂ falls about 3 mmHg per decade, so a normal absolute value may be abnormal for an older patient.' }),
      numberInput('pao2', 'Measured PaO₂ (optional)', {
        unit: 'mmHg',
        min: 20,
        max: 120,
        step: 1,
        helpText: 'Optional. Leave blank to show age-expected PaO₂ only; comparison runs only after a measured value is entered (room air assumed).',
        required: false,
      }),
  ...
  },
  {
    ...
    inputs: [
      yesNo('priorVte', 'Previous VTE (+3)', 3, 'Previous VTE (DVT or PE) with no ongoing anticoagulation (+3 points, the heaviest item).', false),
      yesNo('thrombophilia', 'Known thrombophilia (+2)', 2, 'Documented hereditary (FVL, PT G20210A, AT/PC/PS) or acquired (APLS) — not family history alone.', false),
      yesNo('paralysis', 'Lower-limb paralysis (+2)', 2, 'Paraplegia, dense hemiplegia affecting a leg, or other lower-limb paralysis (not isolated neuropathy).', false),
      yesNo('cancer', 'Active / current cancer (+2)', 2, 'Current/under treatment, diagnosed within ~6 months, metastatic, or palliative.', true),
      yesNo('immobile', 'Immobilized ≥ 7 days (+1)', 1, 'Confined to bed or chair ≥7 consecutive days, including immediately before admission.', true),
      yesNo('icu', 'ICU / CCU stay (+1)', 1, 'ICU or CCU stay during the current admission (+1 point).', false),
      yesNo('age60', 'Age > 60 years (+1)', 1, 'Age above 60 years (+1 point); 60 itself does not score (the model uses >60).', true),
    ],
    calculate(values) {
      const pts: [string, number][] = [
        ['priorVte', 3],
  ...
  },
  {
    ...
    inputs: [
      selectInput('age', 'Age', [
        ...
        { label: '≥ 80 (+2)', value: 2 },
      ], 1, 'Age band: under 60 years 0, 60–79 +1, 80 or older +2 points.'),
      yesNo('priorVte', 'Previous DVT or PE (+2)', 2, 'Previous DVT or PE (+2 points); the original Geneva weights prior VTE lower than the revised version.', false),
      yesNo('surgery', 'Recent surgery (within 4 weeks) (+3)', 3, 'Surgery within 4 weeks, or surgery scheduled in the previous week (+3 points) — the largest single item in the original Geneva model.', true),
      yesNo('hr', 'Heart rate > 100 (+1)', 1, 'Heart rate above 100 beats/min (+1 point) on the ECG or vitals used for the assessment.', true),
      selectInput('paco2', 'PaCO₂ (mmHg)', [
        { label: '≥ 39 (0)', value: 0 },
        { label: '36–38.9 (+1)', value: 1 },
        { label: '< 36 (+2)', value: 2 },
      ], 1, 'Room-air arterial blood gas when feasible (original Geneva derivation).'),
      selectInput('pao2', 'PaO₂ (mmHg)', [
        { label: '≥ 82.5 (0)', value: 0 },
        { label: '71.3–82.4 (+1)', value: 1 },
  ...
  },
  {
    ...
    inputs: [
      numberInput('age', 'Age (for age-adjusted CCI)', { unit: 'years', min: 0, max: 120, step: 1, exampleValue: 65, helpText: 'Age in years for the optional age-adjusted index: each decade over 40 adds 1 point (50 = 1, 60 = 2, 70 = 3, 80 = 4).' }),
      yesNo('mi', 'Myocardial infarction (+1)', 1, 'Documented MI (history or ECG/enzyme evidence), not isolated angina.', true),
      yesNo('chf', 'Congestive heart failure (+1)', 1, 'Symptomatic HF that has required treatment and responded (exertional or paroxysmal nocturnal dyspnea).', false),
      yesNo('pvd', 'Peripheral vascular disease (+1)', 1, 'Intermittent claudication, bypass, gangrene, or untreated aortic aneurysm ≥6 cm.', false),
      yesNo('cva', 'CVA or TIA without hemiplegia (+1)', 1, 'Cerebrovascular disease such as TIA or stroke with minor or no residual deficit. If hemiplegia/paraplegia is present, select the separate +2 hemiplegia item instead; the higher tier supersedes this +1 item.', false),
      yesNo('dementia', 'Dementia (+1)', 1, 'Chronic cognitive deficit interfering with daily life (not isolated acute delirium).', false),
      yesNo('copd', 'Chronic pulmonary disease (+1)', 1, 'Symptomatic chronic lung disease (COPD, asthma requiring treatment, ILD, etc.).', true),
      yesNo('ctd', 'Connective tissue disease (+1)', 1, 'SLE, RA, polymyositis, mixed CTD, or similar systemic rheumatic disease.', false),
      yesNo('pud', 'Peptic ulcer disease (+1)', 1, 'Any history of peptic ulcer (treated or not), including remote.', false),
      yesNo('liverMild', 'Mild liver disease (+1)', 1, 'Chronic hepatitis or cirrhosis without portal hypertension. Score only the highest liver tier.', false),
      yesNo('dm', 'Diabetes without end-organ damage (+1)', 1, 'On insulin or oral agents, not diet alone. Score only the highest diabetes tier.', true),
      yesNo('dmEnd', 'Diabetes with end-organ damage (+2)', 2, 'Retinopathy, neuropathy, or nephropathy. Use this instead of the +1 diabetes item.', false),
      yesNo('hemiplegia', 'Hemiplegia or paraplegia (+2)', 2, 'Hemiplegia or paraplegia with dense residual deficit. This higher CCI tier supersedes cerebrovascular disease/CVA/TIA (+1) when both are documented; do not double-count.', false),
      yesNo('ckd', 'Moderate–severe CKD (+2)', 2, 'Creatinine >3 mg/dL, dialysis, transplant, or uremia — not ordinary stage 3 CKD without that threshold.', false),
      yesNo('tumor', 'Solid tumor (localized) (+2)', 2, 'Treated in the last 5 years; exclude non-melanoma skin cancer. Metastatic tumor is the separate +6 item.', false),
      yesNo('leukemia', 'Leukemia (+2)', 2, 'Acute or chronic leukemia.', false),
      yesNo('lymphoma', 'Lymphoma (+2)', 2, 'Hodgkin or non-Hodgkin lymphoma (not a localized solid tumor).', false),
      yesNo('liverSevere', 'Moderate–severe liver disease (+3)', 3, 'Portal hypertension, varices, or hepatic failure. Use this instead of mild liver disease.', false),
      yesNo('mets', 'Metastatic solid tumor (+6)', 6, 'Metastatic solid tumor (+6, the highest comorbidity weight). Exclude localized or in-situ disease and history of a treated primary without metastases.', false),
      yesNo('aids', 'AIDS (+6)', 6, 'AIDS-defining illness — not asymptomatic HIV without AIDS.', false),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      ], 'none', 'PaO₂/FiO₂ points apply only if ventilated or on CPAP (Le Gall 1993).'),
      ...
      selectInput('chronic', 'Chronic disease', [
        ...
        { label: 'Hematologic malignancy (10)', value: 10, points: 10, description: 'Lymphoma, acute leukemia, or multiple myeloma (Le Gall chronic-disease definition).' },
        { label: 'AIDS (17)', value: 17, points: 17, description: 'HIV with an AIDS-defining clinical complication (e.g. PCP, toxoplasmosis, Kaposi sarcoma, TB) — not asymptomatic HIV.' },
      ], 0, 'Score only the single highest-weighted chronic condition (AIDS 17 > hematologic 10 > metastatic 9 > none 0).'),
      selectInput('admit', 'Type of admission', [
        { label: 'Scheduled surgical (0)', value: 0, points: 0, description: 'Operation planned ≥24 hours in advance' },
        { label: 'Medical (6)', value: 6, points: 6, description: 'No surgery this ICU admission (medical reason for ICU)' },
        { label: 'Unscheduled surgical (8)', value: 8, points: 8, description: 'Emergency / unscheduled operation' },
      ], 6, 'Scheduled = surgery planned ≥24 hours ahead; unscheduled surgical = emergency operation; medical = no surgery this admission.'),
    ],
    calculate(values) {
      const age = num(values.age, 65);
  ...
  },
  {
    ...
    inputs: [
      selectInput('neuro', 'Neurologic (GCS)', [
        { label: '14–15 (0)', value: 0, description: 'GCS 14–15' },
        { label: '9–13 (1)', value: 1, description: 'GCS 9–13' },
        { label: '6–8 (3)', value: 3, description: 'GCS 6–8' },
        { label: '3–5 (5)', value: 5, description: 'GCS 3–5' },
      ], 0, 'Worst GCS. If sedated, use estimated pre-sedation GCS (do not assign verbal 1 solely for the tube).'),
      selectInput('cv', 'Cardiovascular (HR and SBP bands)', [
        { label: 'Normal range (0)', value: 0, description: 'HR 30–139 and SBP 90–239 (score the worse of HR or SBP)' },
        { label: 'Mild derangement (1)', value: 1, description: 'HR ≥140 or SBP 70–89 or SBP 240–269' },
        { label: 'Moderate (3)', value: 3, description: 'SBP 40–69 or SBP ≥270' },
        { label: 'Severe shock-range (5)', value: 5, description: 'HR <30 or SBP <40' },
      ...
      ], 1, 'Official LODS pulmonary points are 0 / 1 / 3 only (Le Gall Table 1).'),
      ...
      ], 1, 'Score the worse of WBC or platelets (Le Gall Table 1). Official hematologic points are 0 / 1 / 3 only.'),
      ...
      ], 0, 'Official LODS hepatic points are 0 or 1 only (Le Gall Table 1).'),
    ],
    calculate(values) {
      const score =
        num(values.neuro) +
  ...
  },
  {
    ...
    inputs: [
      selectInput('resp', 'Respiratory (PaO₂/FiO₂)', [
        { label: '> 300 (0)', value: 0, description: 'PaO₂/FiO₂ >300 mmHg' },
        { label: '226–300 (1)', value: 1, description: 'PaO₂/FiO₂ 226–300 mmHg' },
        { label: '151–225 (2)', value: 2, description: 'PaO₂/FiO₂ 151–225 mmHg' },
        { label: '76–150 (3)', value: 3, description: 'PaO₂/FiO₂ 76–150 mmHg' },
        { label: '≤ 75 (4)', value: 4, description: 'PaO₂/FiO₂ ≤75 mmHg' },
      ], 1, 'Marshall respiratory domain uses PaO₂/FiO₂ in mmHg regardless of PEEP (not PEEP-adjusted).'),
      selectInput('renal', 'Renal (creatinine mg/dL)', [
        { label: '≤ 1.1 (0)', value: 0, description: 'Creatinine ≤1.1 mg/dL (≤100 µmol/L)' },
        { label: '1.2–2.2 (1)', value: 1, description: 'Creatinine 1.2–2.2 mg/dL (101–200 µmol/L)' },
        { label: '2.3–3.9 (2)', value: 2, description: 'Creatinine 2.3–3.9 mg/dL (201–350 µmol/L)' },
        { label: '4.0–5.6 (3)', value: 3, description: 'Creatinine 4.0–5.6 mg/dL (351–500 µmol/L)' },
        { label: '≥ 5.7 (4)', value: 4, description: 'Creatinine ≥5.7 mg/dL (>500 µmol/L)' },
      ], 1, 'Serum creatinine. Marshall original cuts are the µmol/L bands shown in the descriptors.'),
      selectInput('hepatic', 'Hepatic (bilirubin mg/dL)', [
        { label: '≤ 1.2 (0)', value: 0, description: 'Bilirubin ≤1.2 mg/dL (≤20 µmol/L)' },
        { label: '1.3–3.5 (1)', value: 1, description: 'Bilirubin 1.3–3.5 mg/dL (21–60 µmol/L)' },
        { label: '3.6–7.0 (2)', value: 2, description: 'Bilirubin 3.6–7.0 mg/dL (61–120 µmol/L)' },
        { label: '7.1–14.0 (3)', value: 3, description: 'Bilirubin 7.1–14.0 mg/dL (121–240 µmol/L)' },
        { label: '> 14.0 (4)', value: 4, description: 'Bilirubin >14.0 mg/dL (>240 µmol/L)' },
      ], 1, 'Total bilirubin. 1 mg/dL ≈ 17.1 µmol/L.'),
      ...
      ], 1, 'PAR = HR (beats/min) × CVP (mmHg) ÷ MAP (mmHg). Example: HR 100, CVP 12, MAP 70 → PAR 17.1 → 2 points. If no CVP, this domain cannot be scored from vitals alone — do not invent a pressor-dose table; high-dose pressors is an educational surrogate for PAR >30 only.'),
      selectInput('heme', 'Hematologic (platelets ×10³/µL)', [
        { label: '> 120 (0)', value: 0, description: 'Platelets >120 ×10³/µL' },
        { label: '81–120 (1)', value: 1, description: 'Platelets 81–120 ×10³/µL' },
        { label: '51–80 (2)', value: 2, description: 'Platelets 51–80 ×10³/µL' },
        { label: '21–50 (3)', value: 3, description: 'Platelets 21–50 ×10³/µL' },
        { label: '≤ 20 (4)', value: 4, description: 'Platelets ≤20 ×10³/µL' },
      ], 1, 'Lowest platelet count in ×10³/µL during the 24-hour period: >120 scores 0, 81–120 1, 51–80 2, 21–50 3, ≤20 4 points.'),
      selectInput('neuro', 'Neurologic (GCS)', [
        { label: '15 (0)', value: 0, description: 'GCS 15' },
        { label: '13–14 (1)', value: 1, description: 'GCS 13–14' },
        { label: '10–12 (2)', value: 2, description: 'GCS 10–12' },
        { label: '7–9 (3)', value: 3, description: 'GCS 7–9' },
        { label: '≤ 6 (4)', value: 4, description: 'GCS ≤6' },
      ], 0, 'Worst GCS. If sedated/intubated, estimate the pre-sedation GCS (do not assign verbal 1 solely for the tube).'),
    ],
    calculate(values) {
      const score =
        num(values.resp) +
  ...
  },
  {
    ...
    inputs: [
      yesNo('infection', 'Suspected or documented infection', 0, 'Suspected or documented infection (clinical syndrome plus cultures/imaging/labs; no single test confirms it) — required before septic shock can be assigned.', true),
      yesNo('fluids', 'Adequate volume resuscitation (e.g. ~30 mL/kg crystalloid unless fluid-intolerant / already volume-replete)', 0, 'Sepsis-3 leaves volume unspecified; SSC operationalizes ~30 mL/kg crystalloid (or clinically adequate preload) before the shock label.', false),
      yesNo('vasopressors', 'Vasopressors required to maintain MAP ≥ 65 mmHg', 0, 'Yes when vasopressors are needed to keep MAP at 65 mmHg or higher despite adequate volume resuscitation — the defining circulatory element of septic shock.', false),
      ...
      numberInput('map', 'Current MAP (optional context)', { unit: 'mmHg', min: 0, max: 150, step: 1, exampleValue: 65, required: false, helpText: 'Optional current MAP in mmHg for context; the septic-shock definition uses the vasopressor requirement (MAP ≥65), and this field is not itself part of the criteria.' }),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      numberInput('pct', 'Procalcitonin', { unit: 'ng/mL', min: 0, max: 200, step: 0.01, exampleValue: 0.25, helpText: 'Procalcitonin in ng/mL from a validated assay. Bands: <0.1 bacterial LRTI unlikely; 0.1–0.25 unlikely/borderline; >0.25–0.5 possible; >0.5 likely. Clinical instability or high suspicion overrides the band.' }),
      ...
      ], 'lrti', 'PCT is an adjunct. Do not withhold antibiotics for shock, severe immunodeficiency, or very high clinical suspicion.'),
      yesNo('unstable', 'Hemodynamic instability or severe immunodeficiency (override)', 0, 'Shock, vasopressors, or severe immunocompromise — treat clinically; ignore a “low” PCT.', false),
      yesNo('highSuspicion', 'Very high clinical suspicion for bacterial infection (override)', 0, 'Clinician judges bacterial infection very likely (e.g. purulent focus, bacteremia suspected) regardless of PCT.', false),
    ],
    calculate(values) {
      ...
      selectInput('riskGroup', 'Highest applicable risk group', [
        {
          ...
          value: 'high5',
          description:
            'CDC ≥5 mm: HIV; recent TB contacts; fibrotic CXR consistent with prior TB; organ transplant; other immunosuppressed (TNF-α antagonists, chemotherapy, or prednisone ≥15 mg/day for ≥1 month).',
        },
        {
          label: 'Moderate risk — use ≥10 mm cutoff (immigrants, IVDU, congregate settings, lab workers, clinical risks, children <5, etc.)',
          value: 'mod10',
          description:
            'CDC ≥10 mm: immigrants (<5 y) from high-prevalence countries; injection-drug users; residents/employees of prisons, nursing homes, hospitals, shelters; mycobacteriology lab staff; silicosis, diabetes, CKD, leukemias/lymphomas, head/neck/lung cancer, weight loss ≥10% IBW, gastrectomy/bypass; children <4 y; children/adolescents exposed to high-risk adults.',
        },
        {
          label: 'Low risk — use ≥15 mm cutoff (no TB risk factors)',
          value: 'low15',
          description: 'CDC ≥15 mm: persons with no known TB risk factors. Do not use this cutoff if any ≥5 or ≥10 mm risk is present — pick the highest-risk applicable group.',
        },
      ], 'mod10', 'Choose the highest-risk group that applies (a transplant patient is ≥5 mm even if they also have other risks). Measure induration, not erythema.'),
      yesNo('bcg', 'Prior BCG vaccination (context)', 0, 'Prior BCG vaccination makes a positive TST harder to interpret but does not by itself change the cutoff — do not exclude or downgrade a positive result on BCG history alone.', true),
      yesNo('igraPrefer', 'IGRA preferred / available in this context', 0, 'Yes when an IGRA is available and preferred (BCG-vaccinated patients, or when a confirmatory test is needed); an IGRA is not affected by prior BCG.', true),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('abx60', 'Antibiotic use within 60 days (major +2)', 2, 'Any systemic antibiotic in the past 60 days (not topical).', false),
      yesNo('ltc', 'Long-term care facility residence (major +2)', 2, 'Skilled nursing/LTC, not assisted living.', false),
      yesNo('tubeFeed', 'Tube feeding (major +2)', 2, 'Any enteral feeding tube (NG, NJ, PEG) in use at evaluation.', false),
      yesNo('priorDrp', 'Prior drug-resistant pneumonia — MRSA or Pseudomonas (major +2)', 2, 'MRSA or Pseudomonas infection in the past 1 year.', false),
      yesNo('hosp60', 'Hospitalization within 60 days (minor +1)', 1, 'Overnight hospital stay in the past 60 days.', false),
      yesNo('chronicLung', 'Chronic pulmonary disease (minor +1)', 1, 'COPD or other chronic lung disease (e.g. bronchiectasis, ILD) — not isolated well-controlled asthma in most DRIP uses.', true),
      yesNo('poorFunc', 'Poor functional status (minor +1)', 1, 'Karnofsky <70 or nonambulatory / dependent for ADLs.', false),
      yesNo('acidSuppression', 'Gastric acid suppression (minor +1)', 1, 'PPI or H2-blocker (e.g. within 14 days).', true),
      yesNo('woundCare', 'Wound care (minor +1)', 1, 'Active skilled wound care (e.g. within 30 days).', false),
      yesNo('mrsaCol', 'MRSA colonization (minor +1)', 1, 'Documented MRSA colonization in the past 1 year.', false),
    ],
    calculate(values) {
