...
export const wave2PulmIdCalcs: Calculator[] = [
  {
    ...
    inputs: [
      yesNo('shock', 'Septic shock requiring vasopressors (major)', 1, 'Infection-related shock needing vasopressors to maintain BP — this single major criterion defines severe CAP.'),
      yesNo('vent', 'Respiratory failure requiring mechanical ventilation (major)', 1, 'Invasive mechanical ventilation for respiratory failure (the other major criterion).'),
      yesNo('rr', 'Respiratory rate ≥ 30/min (minor)', 1),
      yesNo('pf', 'PaO₂/FiO₂ ≤ 250 (minor)', 1, 'PaO₂/FiO₂ ≤250 mmHg (PaO₂ in mmHg ÷ FiO₂ as a fraction).'),
      yesNo('multilobar', 'Multilobar infiltrates (minor)', 1, 'Infiltrates in ≥2 lobes on CXR or CT.'),
      yesNo('confusion', 'Confusion / disorientation (minor)', 1, 'New disorientation to person/place/time (or equivalent). If baseline dementia, score only acute change.'),
      ...
      yesNo('hypotension', 'Hypotension requiring aggressive fluid resuscitation (minor)', 1, 'SBP <90 mmHg or MAP <65 that requires aggressive fluids to restore BP. If vasopressors are needed, score the major septic-shock criterion instead.'),
    ],
    calculate(values) {
      const major =
  ...
  },
  {
    ...
    inputs: [
      ...
      yesNo('ams', 'Altered mental status — new disorientation to person/place/time or GCS <15 (+5)', 5, 'If baseline dementia, score only acute change. España AMS is new confusion, not chronic cognitive impairment without change.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      selectInput('comorbid', 'Number of comorbidities', [
        ...
        { label: '1 (1)', value: 1 },
        { label: '≥ 2 (2)', value: 2 },
      ], undefined, 'Count chronic cardiac; chronic respiratory excluding asthma; chronic renal (eGFR ≤30); liver disease; dementia; chronic neurological; connective tissue; diabetes; HIV/AIDS; malignancy; clinician-defined obesity. Do not count asthma alone.'),
      selectInput('rr', 'Respiratory rate (breaths/min)', [
        { label: '< 20 (0)', value: 0 },
        { label: '20–29 (1)', value: 1 },
        { label: '≥ 30 (2)', value: 2 },
      ], undefined, 'Admission respiratory rate (breaths/min).'),
      selectInput('spo2', 'SpO₂ on room air', [
        { label: '≥ 92% (0)', value: 0 },
        { label: '< 92% (2)', value: 2 },
      ], undefined, 'Admission SpO₂ on room air when feasible (ISARIC 4C points assume room air).'),
      selectInput('gcs', 'GCS', [
        { label: '15 (0)', value: 0, description: 'GCS 15' },
        { label: '< 15 (2)', value: 2, description: 'GCS <15 from any cause, including new confusion' },
      ], undefined, 'Admission GCS. Score 2 if GCS <15 (including new confusion). If sedated, estimate pre-sedation GCS.'),
      ...
      ], undefined, 'ISARIC 4C: urea <7 mmol/L = 0 points; urea exactly 7 mmol/L scores 1.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Divide the CXR into 4 quadrants (right/left × upper/lower, split at the hilum). Count quadrants with alveolar consolidation — not interstitial markings alone.'),
      ...
      ], undefined, 'PaO₂/FiO₂ in mmHg (PaO₂ in mmHg ÷ FiO₂ as a fraction).'),
      ...
      ], undefined, 'Set PEEP at the time of scoring (cm H₂O). If not ventilated, this component is typically 0.'),
      ...
      ], undefined, 'Crs = VT (mL) / (Pplat − PEEP) in mL/cm H₂O. Use static (inspiratory-hold) compliance when available.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      yesNo('thrombophilia', 'Known thrombophilia (+2)', 2, 'Documented hereditary (FVL, PT G20210A, AT/PC/PS) or acquired (APLS) — not family history alone.'),
      yesNo('paralysis', 'Lower-limb paralysis (+2)', 2, 'Paraplegia, dense hemiplegia affecting a leg, or other lower-limb paralysis (not isolated neuropathy).'),
      yesNo('cancer', 'Active / current cancer (+2)', 2, 'Current/under treatment, diagnosed within ~6 months, metastatic, or palliative.'),
      yesNo('immobile', 'Immobilized ≥ 7 days (+1)', 1, 'Confined to bed or chair ≥7 consecutive days, including immediately before admission.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      selectInput('paco2', 'PaCO₂ (mmHg)', [
        ...
        { label: '< 36 (+2)', value: 2 },
      ], undefined, 'Room-air arterial blood gas when feasible (original Geneva derivation).'),
      selectInput('pao2', 'PaO₂ (mmHg)', [
        ...
        { label: '< 49 (+4)', value: 4 },
      ], undefined, 'Wicki 2001 PaO₂ bands from kPa: ≥11 kPa (≥82.5 mmHg)=0; 9.5–10.99 kPa (71.3–82.4)=+1; 8–9.49 kPa (60–71.2)=+2. Room-air ABG when feasible.'),
      yesNo('atelectasis', 'Plate-like atelectasis on CXR (+1)', 1, 'Linear/plate-like atelectasis on the chest radiograph (original Geneva CXR item).'),
      yesNo('diaphragm', 'Elevation of hemidiaphragm on CXR (+1)', 1, 'Unilateral hemidiaphragm elevation on CXR (original Geneva CXR item).'),
    ],
    calculate(values) {
      const score =
        num(values.age) +
  ...
  },
  {
    ...
    inputs: [
      ...
      yesNo('pud', 'Peptic ulcer disease (+1)', 1, 'Any history of peptic ulcer (treated or not), including remote.'),
      ...
      yesNo('lymphoma', 'Lymphoma (+2)', 2, 'Hodgkin or non-Hodgkin lymphoma (not a localized solid tumor).'),
      yesNo('liverSevere', 'Moderate–severe liver disease (+3)', 3, 'Portal hypertension, varices, or hepatic failure. Use this instead of mild liver disease.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Score only the single highest-weighted chronic condition (AIDS 17 > hematologic 10 > metastatic 9 > none 0).'),
      ...
      ], undefined, 'Scheduled = surgery planned ≥24 hours ahead; unscheduled surgical = emergency operation; medical = no surgery this admission.'),
    ],
    calculate(values) {
      const age = num(values.age, 65);
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Worst GCS. If sedated, use estimated pre-sedation GCS (do not assign verbal 1 solely for the tube).'),
      ...
      ], undefined, 'Le Gall Table 1: take the worse of heart rate or systolic BP.'),
      ...
      ], undefined, 'Le Gall Table 1: take the worst of urea, creatinine, or 24-h urine output.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Marshall respiratory domain uses PaO₂/FiO₂ in mmHg regardless of PEEP (not PEEP-adjusted).'),
      ...
      ], undefined, 'Serum creatinine. Marshall original cuts are the µmol/L bands shown in the descriptors.'),
      ...
      ], undefined, 'Total bilirubin. 1 mg/dL ≈ 17.1 µmol/L.'),
      ...
      ], undefined, 'PAR = HR (beats/min) × CVP (mmHg) ÷ MAP (mmHg). Example: HR 100, CVP 12, MAP 70 → PAR 17.1 → 2 points. If no CVP, this domain cannot be scored from vitals alone — do not invent a pressor-dose table; high-dose pressors is an educational surrogate for PAR >30 only.'),
      ...
      ], undefined, 'Worst GCS. If sedated/intubated, estimate the pre-sedation GCS (do not assign verbal 1 solely for the tube).'),
    ],
    calculate(values) {
      const score =
        num(values.resp) +
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'PCT is an adjunct. Do not withhold antibiotics for shock, severe immunodeficiency, or very high clinical suspicion.'),
      yesNo('unstable', 'Hemodynamic instability or severe immunodeficiency (override)', 0, 'Shock, vasopressors, or severe immunocompromise — treat clinically; ignore a “low” PCT.'),
      yesNo('highSuspicion', 'Very high clinical suspicion for bacterial infection (override)', 0, 'Clinician judges bacterial infection very likely (e.g. purulent focus, bacteremia suspected) regardless of PCT.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Choose the highest-risk group that applies (a transplant patient is ≥5 mm even if they also have other risks). Measure induration, not erythema.'),
  ...
  },
  {
    ...
    inputs: [
      yesNo('abx60', 'Antibiotic use within 60 days (major +2)', 2, 'Any systemic antibiotic in the past 60 days (not topical).'),
      yesNo('ltc', 'Long-term care facility residence (major +2)', 2, 'Skilled nursing/LTC, not assisted living.'),
      yesNo('tubeFeed', 'Tube feeding (major +2)', 2, 'Any enteral feeding tube (NG, NJ, PEG) in use at evaluation.'),
      yesNo('priorDrp', 'Prior drug-resistant pneumonia — MRSA or Pseudomonas (major +2)', 2, 'MRSA or Pseudomonas infection in the past 1 year.'),
      yesNo('hosp60', 'Hospitalization within 60 days (minor +1)', 1, 'Overnight hospital stay in the past 60 days.'),
      yesNo('chronicLung', 'Chronic pulmonary disease (minor +1)', 1, 'COPD or other chronic lung disease (e.g. bronchiectasis, ILD) — not isolated well-controlled asthma in most DRIP uses.'),
      yesNo('poorFunc', 'Poor functional status (minor +1)', 1, 'Karnofsky <70 or nonambulatory / dependent for ADLs.'),
      yesNo('acidSuppression', 'Gastric acid suppression (minor +1)', 1, 'PPI or H2-blocker (e.g. within 14 days).'),
      yesNo('woundCare', 'Wound care (minor +1)', 1, 'Active skilled wound care (e.g. within 30 days).'),
      yesNo('mrsaCol', 'MRSA colonization (minor +1)', 1, 'Documented MRSA colonization in the past 1 year.'),
    ],
    calculate(values) {
