...
export const wave2CardiologyCalcs: Calculator[] = [
  {
    ...
    inputs: [
      ...
      ], undefined, 'Count: treated HTN, hyperlipidemia, DM, BMI >30, current/recent smoking, first-degree CAD/MI/SCD before 65. Known atherosclerotic disease scores 2 regardless of risk-factor count.'),
      selectInput('troponinPoints', 'Initial troponin (HEART component)', [
        ...
        { label: '1–3× normal limit', value: 1, points: 1, description: 'Above URL up to 3× the local 99th-percentile URL' },
        { label: '> 3× normal limit', value: 2, points: 2, description: 'More than 3× the local 99th-percentile URL' },
      ], 0, 'Use the local assay upper reference limit (99th percentile), not a “chest pain panel” cutoff. This is the initial troponin HEART component; serial negatives are a separate pathway item.'),
  ...
  },
  {
    ...
    inputs: [
      selectInput(
        ...
        [
          { label: 'CPC 1 — Good cerebral performance', value: 1, description: 'Conscious, alert, able to work and lead a normal life; may have mild neurologic or psychological deficit' },
          { label: 'CPC 2 — Moderate cerebral disability', value: 2, description: 'Conscious; independent in ADLs (dress, travel, prepare food); may work in a sheltered environment or have hemiplegia/seizures/ataxia' },
          { label: 'CPC 3 — Severe cerebral disability', value: 3, description: 'Conscious but dependent on others for daily support because of impaired brain function (institutional or at home)' },
          { label: 'CPC 4 — Coma or vegetative state', value: 4, description: 'Unaware of surroundings; no verbal or psychological interaction with the environment; may have sleep–wake cycles' },
          { label: 'CPC 5 — Brain death / death', value: 5, description: 'Certified brain death or dead' },
        ],
        1,
        'Assign at a defined time point (usually hospital discharge). CPC 1–2 are “favorable” in most OHCA trials. Based on cerebral function, not extra-cerebral disability alone.',
      ),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      selectInput('ntprobnp', 'NT-proBNP category (biomarker)', [
        ...
        { label: 'Markedly elevated (>1000 ng/L)', value: 2, points: 2, description: 'Upper tertile-style band; ARISTOTLE IQR upper bound ~1250 ng/L' },
      ], undefined, 'Educational tertile-style bands (Hijazi ABC-stroke / ARISTOTLE), not the official continuous ABC model. Units ng/L = pg/mL.'),
      selectInput('hstn', 'hs-Troponin category (biomarker)', [
        { label: 'Low / near LoD (hs-cTnT typically ≤7 ng/L)', value: 0, points: 0, description: 'Near assay limit of detection; ARISTOTLE lower quartile ~7.5 ng/L' },
        { label: 'Moderately elevated (around URL, hs-cTnT ~8–14 ng/L)', value: 1, points: 1, description: 'Around the 99th-percentile URL (~14 ng/L for hs-cTnT)' },
        { label: 'Markedly elevated (several-fold URL, hs-cTnT typically >14 ng/L)', value: 2, points: 2, description: 'Above URL; ARISTOTLE upper quartile ~16.7 ng/L and higher' },
      ], undefined, 'Educational tertile proxy (hs-cTnT shown). Not the published continuous ABC calculator.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      selectInput('hb', 'Hemoglobin category', [
        ...
        { label: 'Moderate–severe anemia (<11 g/dL)', value: 2, points: 2, description: 'WHO moderate–severe: Hb <11 g/dL (either sex)' },
      ], undefined, 'Educational WHO-style Hb bands (ABC-bleed uses continuous hemoglobin). Keep educational disclaimer.'),
      ...
      selectInput('gdf', 'GDF-15 category (if known)', [
        ...
        { label: 'Markedly elevated (>1470 ng/L)', value: 2, points: 2, description: 'ARISTOTLE upper tertile (IQR upper ~2052 ng/L and above)' },
      ], undefined, 'GDF-15 is often research-only; optional educational field. Tertile-style bands from ARISTOTLE ABC-bleed derivation — not the continuous official model.'),
    ],
    calculate(values) {
      const score =
        num(values.age) +
  ...
  },
  {
    ...
    inputs: [
      ...
      selectInput(
        ...
        'LV function',
        [
          { label: 'Good (EF ≥51%)', value: 0, description: 'LVEF ≥51%' },
          { label: 'Moderate (EF 31–50%)', value: 1, description: 'LVEF 31–50%' },
          { label: 'Poor (EF 21–30%)', value: 2, description: 'LVEF 21–30%' },
          { label: 'Very poor (EF ≤20%)', value: 3, description: 'LVEF ≤20%' },
        ],
        0,
        'EuroSCORE II LV function bands from measured EF (echo/cath). Use the most recent pre-op EF.',
      ),
