...
export const wave7BedsideCalcs: Calculator[] = [
  // ─── 1. CKiD U25 eGFR ──────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      numberInput('height', 'Height', { unit: 'cm', min: 50, max: 220, step: 0.1, defaultValue: 140 }),
  ...
  // ─── 2. Body Roundness Index ───────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      numberInput('waist', 'Waist circumference', { unit: 'cm', min: 40, max: 200, step: 0.1, defaultValue: 80 }),
    ],
    calculate(values) {
  ...
  // ─── 3. ROX index ──────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('spo2', 'SpO₂', { unit: '%', min: 50, max: 100, step: 1, defaultValue: 98 }),
      ...
      numberInput('rr', 'Respiratory rate', { unit: '/min', min: 8, max: 50, step: 1, defaultValue: 20 }),
      selectInput(
        'timepoint',
  ...
  // ─── 4. HACOR ──────────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('hr', 'Heart rate', { unit: '/min', min: 40, max: 180, step: 1, defaultValue: 90 }),
      numberInput('ph', 'Arterial pH', { min: 6.8, max: 7.6, step: 0.01, defaultValue: 7.36 }),
      numberInput('gcs', 'Glasgow Coma Scale', { min: 3, max: 15, step: 1, defaultValue: 15 }),
      numberInput('pf', 'PaO₂/FiO₂', { unit: 'mmHg', min: 40, max: 600, step: 1, defaultValue: 220 }),
      numberInput('rr', 'Respiratory rate', { unit: '/min', min: 8, max: 60, step: 1, defaultValue: 24 }),
    ],
    calculate(values) {
  ...
  // ─── 5. 2023 Duke-ISCVID IE criteria ───────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('surgicalMajor', 'Major surgical: intraoperative inspection with evidence of IE', null),
      yesNo('pathologic', 'Pathologic criteria: microorganisms or active endocarditis on vegetation, explanted valve/CIED, or embolus', null),
      yesNo(
        'predisposition',
        'Minor predisposition: prior IE, prosthetic/TAVR valve, valve repair, CHD, CIED, HOCM, >mild native valve disease, IVDU, LVAD/MCS',
        null,
      ),
      yesNo('fever', 'Minor: fever ≥38.0 °C', null),
      yesNo(
        'vascular',
      ...
      yesNo('immuno', 'Minor immunologic: Osler nodes, Roth spots, glomerulonephritis, rheumatoid factor', null),
      yesNo('microMinor', 'Minor microbiology: positive cultures or serology not meeting major definitions', null),
    ],
    calculate(values) {
  ...
  // ─── 6. H2FPEF ─────────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('af', 'Atrial fibrillation (paroxysmal or persistent)', 3),
    ...
    inputs: [
      selectInput(
        ...
        [
          ...
          { label: 'Minor only (e.g. E/e′ 9–14, GLS <16%)', value: 1, points: 1 },
          { label: 'Major (septal e′<7 or lateral e′<10, or E/e′≥15, or TR>2.8 m/s)', value: 2, points: 2 },
        ],
        0,
        'Score major OR minor in this domain (not both). Major = 2, minor = 1.',
      ),
      selectInput(
        'morphological',
        ...
        [
          { label: 'None', value: 0, points: 0 },
          { label: 'Minor only (LAVI 29–34, LVMI ≥115/95 M/F, RWT>0.42, WT≥12 mm)', value: 1, points: 1 },
          { label: 'Major (LAVI>34 mL/m², or LVMI ≥149/122 and RWT>0.42)', value: 2, points: 2 },
        ],
        0,
      ),
      selectInput(
        ...
        [
          { label: 'None', value: 0, points: 0 },
          { label: 'Minor (NT-proBNP 125–220 SR / 365–660 AF; BNP 35–80 / 105–240)', value: 1, points: 1 },
          { label: 'Major (NT-proBNP >220 SR / >660 AF; BNP >80 / >240)', value: 2, points: 2 },
        ],
        0,
      ),
    ],
  ...
  // ─── 8. PECARN CSI ─────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('gcsUnresponsive', 'High-risk: GCS 3–8 or unresponsive (AVPU = U)', null),
      yesNo('abnormalAbc', 'High-risk: abnormal airway, breathing, or circulation', null),
      yesNo('focalNeuro', 'High-risk: focal neurologic deficit (paresthesia, numbness, or weakness)', null),
      yesNo('ams', 'Intermediate: altered mental status (GCS 9–14, AVPU V/P, or other AMS)', null),
      yesNo('neckPain', 'Intermediate: self-reported neck pain', null),
      yesNo('midlineTenderness', 'Intermediate: posterior midline neck tenderness', null),
      yesNo('substantialHead', 'Intermediate: substantial head injury (needs OR or admission)', null),
      yesNo('substantialTorso', 'Intermediate: substantial torso injury (needs OR or admission)', null),
    ],
    calculate(values) {
  ...
  // ─── 9. 2HELPS2B ───────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('birds', 'BIRDs — brief (ictal) rhythmic discharges', 2),
      yesNo('freqGt2', 'Frequency >2 Hz for any periodic or rhythmic pattern', 1),
      yesNo('epileptiform', 'Sporadic epileptiform discharges', 1),
      yesNo('lpdLrdaBipd', 'LPDs, LRDA, or bilateral independent PDs', 1),
      yesNo('plusFeatures', 'Plus features (superimposed fast, rhythmic, or sharp activity)', 1),
      yesNo('priorSeizure', 'Prior seizure (acute or remote, including epilepsy)', 1),
    ],
    calculate(values) {
  ...
  // ─── 10. NUTRIC ────────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('age', 'Age', { unit: 'years', min: 18, max: 100, step: 1, defaultValue: 60 }),
      numberInput('apache', 'APACHE II', { min: 0, max: 50, step: 1, defaultValue: 18 }),
      numberInput('sofa', 'SOFA (initial)', { min: 0, max: 24, step: 1, defaultValue: 5 }),
      numberInput('comorbidities', 'Number of comorbidities', { min: 0, max: 12, step: 1, defaultValue: 1 }),
  ...
  // ─── 11. FAST score ────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('lsm', 'Liver stiffness (VCTE)', { unit: 'kPa', min: 1.5, max: 75, step: 0.1, defaultValue: 8 }),
  ...
  // ─── 12. Agile 3+ ──────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      numberInput('lsm', 'Liver stiffness (VCTE)', { unit: 'kPa', min: 1.5, max: 75, step: 0.1, defaultValue: 10 }),
      numberInput('ast', 'AST', { unit: 'U/L', min: 5, max: 500, step: 1, defaultValue: 40 }),
  ...
  // ─── 13. Agile 4 ───────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('diabetes', 'Diabetes mellitus', null),
      numberInput('lsm', 'Liver stiffness (VCTE)', { unit: 'kPa', min: 1.5, max: 75, step: 0.1, defaultValue: 12 }),
      numberInput('ast', 'AST', { unit: 'U/L', min: 5, max: 500, step: 1, defaultValue: 40 }),
      numberInput('alt', 'ALT', { unit: 'U/L', min: 5, max: 500, step: 1, defaultValue: 45 }),
  ...
  // ─── 14. MASLD criteria ────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('steatosis', 'Hepatic steatosis (imaging, CAP, or histology)', null),
      yesNo('otherLiver', 'Other liver disease that could fully explain steatosis (e.g. HCV, drugs, monogenic)', null),
      yesNo('bmiMet', 'Cardiometabolic: BMI ≥25 kg/m² (≥23 in Asian populations)', null),
      yesNo('waistMet', 'Cardiometabolic: waist ≥94 cm (men) or ≥80 cm (women) — ethnicity-adjusted cutoffs allowed', null),
      yesNo('glucoseMet', 'Cardiometabolic: T2D, prediabetes, HOMA-IR, or HbA1c ≥5.7%', null),
