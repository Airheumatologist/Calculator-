...
export const emergencyMiscCalcs: Calculator[] = [
  {
    ...
    inputs: [
      ...
      yesNo('postMed', 'Bone tenderness posterior distal 6 cm medial malleolus', 0,
        'Palpate bone along the posterior edge or tip of the distal 6 cm of the tibia — not the deltoid ligament or anterior joint.'),
      yesNo('navicular', 'Bone tenderness navicular', 0,
        'Palpate bone, not soft tissue, at the navicular.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      yesNo('flex90', 'Inability to flex knee to 90°', 1,
        'Cannot actively flex the injured knee to 90°. Measure from full extension.'),
  ...
  },
  {
    ...
    inputs: [
      yesNo('midline', 'Midline posterior cervical tenderness', 1,
        'Pain on palpation of the posterior midline from the nuchal ridge to T1, or any cervical spinous process (lateral/paraspinal tenderness does not count).'),
      yesNo('intox', 'Evidence of intoxication', 1,
        'Recent ingestion, exam evidence (odor of alcohol, slurred speech, ataxia), or tests showing alcohol/drugs affecting alertness (e.g. BAL >0.08%).'),
      yesNo('ams', 'Altered level of alertness', 1,
        'GCS ≤14; disoriented to person/place/time/events; cannot remember 3 objects at 5 min; or delayed/inappropriate response to external stimuli.'),
      yesNo('focal', 'Focal neurologic deficit', 1,
        'Any new motor or sensory deficit (including numbness, weakness, or abnormal reflexes).'),
      yesNo('distracting', 'Painful distracting injury', 1,
        'Long-bone fracture; visceral injury needing surgical consult; large laceration/degloving/crush; large burns; or any injury causing acute functional impairment judged able to mask neck pain.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      yesNo('head', 'Head & neck (9%) — entire region', 9,
        'Adult Wallace nines. Check Yes only if the entire region is partial- or full-thickness burn; for partial regions leave No and add a mental fraction of the listed % (patient palm including fingers ≈1%). Exclude first-degree. Children use Lund-Browder.'),
      yesNo('antTrunk', 'Anterior trunk (18%) — entire region', 18,
        'Check Yes only if the entire anterior trunk is 2nd/3rd-degree; otherwise leave No and estimate a fraction of 18%.'),
      yesNo('postTrunk', 'Posterior trunk (18%) — entire region', 18,
        'Check Yes only if the entire posterior trunk is 2nd/3rd-degree; otherwise leave No and estimate a fraction of 18%.'),
      yesNo('armR', 'Right arm (9%) — entire region', 9,
        'Check Yes only if the entire right arm (including hand) is 2nd/3rd-degree; otherwise leave No and estimate a fraction of 9%.'),
      yesNo('armL', 'Left arm (9%) — entire region', 9,
        'Check Yes only if the entire left arm (including hand) is 2nd/3rd-degree; otherwise leave No and estimate a fraction of 9%.'),
      yesNo('legR', 'Right leg (18%) — entire region', 18,
        'Check Yes only if the entire right leg (including foot) is 2nd/3rd-degree; otherwise leave No and estimate a fraction of 18%.'),
      yesNo('legL', 'Left leg (18%) — entire region', 18,
        'Check Yes only if the entire left leg (including foot) is 2nd/3rd-degree; otherwise leave No and estimate a fraction of 18%.'),
      yesNo('perineum', 'Perineum (1%) — entire region', 1,
        'Adult perineum/genitalia ≈1%.'),
    ],
    calculate(values) {
      let tbsa = 0;
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Reflex irritability: response to suction of the nares (or a tap on the sole). Score at 1 and 5 minutes; repeat q5 min if <7.'),
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Digital exam of the internal os, in centimetres.'),
      ...
      ], undefined, 'Effacement = percent shortening of cervical length vs uneffaced (~3–4 cm).'),
      selectInput('station', 'Station', [
        { label: '−3 (0)', value: 0 },
        { label: '−2 (1)', value: 1 },
        { label: '−1 / 0 (2)', value: 2 },
        { label: '+1 / +2 (3)', value: 3 },
      ], undefined, 'Station = cm relative to the ischial spines (original Bishop, not the modified −5…+5 ACOG scale).'),
      ...
      ], undefined, 'Firm ≈ nasal tip, medium ≈ chin, soft ≈ lips.'),
      ...
      ], undefined, 'Position of the cervical os in the vagina — posterior / mid-position / anterior.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Listen without a stethoscope. Agitation = crying or being examined; rest = calm, including sleep.'),
      ...
      ], undefined, 'Central cyanosis of lips/tongue — not isolated acrocyanosis. Agitation vs rest as for stridor.'),
      ...
      ], undefined, 'Asleep but rousable is scored 0. Do not score a sleeping child 5.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Percent fall = (post-heparin peak − nadir) / peak. Nadir is the lowest platelet count. Score the worse of % fall vs nadir band.'),
      ...
      ], undefined, 'Day 0 = the day heparin was started.'),
      ...
      ], undefined, '2 = new confirmed thrombosis, skin necrosis, or acute systemic reaction after IV UFH bolus (not any fever). 1 = progressive/recurrent thrombosis, non-necrotizing skin lesions, or suspected unproven thrombosis.'),
      ...
      ], undefined, 'Definite (0): surgery/IABP/CVVH within 72 h, confirmed bacteremia/DIC, chemotherapy, or another drug known to drop platelets. Possible (1): competing cause plausible. None apparent (2): no alternative.'),
    ],
    calculate(values) {
  ...
  },
  {
    ...
    inputs: [
      ...
      yesNo('priorVte', 'Previous VTE (excluding superficial)', 3,
        'Prior DVT or PE, not superficial thrombophlebitis.'),
      yesNo('reducedMob', 'Reduced mobility', 3,
        'Anticipated bed rest with bathroom privileges (limitation or order) for at least 3 days.'),
      yesNo('thrombophilia', 'Known thrombophilic condition', 3,
        'Antithrombin, protein C or S deficiency, factor V Leiden, prothrombin G20210A, or antiphospholipid syndrome.'),
      ...
      yesNo('heartLung', 'Heart and/or respiratory failure', 1,
        'Acute or decompensated heart failure and/or respiratory failure this admission.'),
      yesNo('acuteMiStroke', 'Acute MI or ischemic stroke', 1,
        'This admission: acute myocardial infarction or ischemic stroke.'),
      yesNo('infectionRheum', 'Acute infection and/or rheumatologic disorder', 1,
        'Acute infection (any site) and/or active rheumatologic disease this admission.'),
      yesNo('obesity', 'Obesity (BMI ≥30)', 1),
      yesNo('hormone', 'Ongoing hormonal treatment', 1,
        'Ongoing estrogen (OCP, HRT) or similar hormonal therapy.'),
    ],
    calculate(values) {
      const score =
  ...
  },
  {
    ...
    inputs: [
      yesNo('snore', 'Snore loudly (heard through closed doors, or partner elbows you)?', 1),
      yesNo('tired', 'Often tired, fatigued, or sleepy in the daytime (e.g. falling asleep while driving or talking)?', 1),
      yesNo('observed', 'Anyone observed you stop breathing, or choke/gasp during sleep?', 1),
      yesNo('pressure', 'High blood pressure (have, or being treated)?', 1),
      yesNo('bmi', 'BMI > 35', 1),
      yesNo('age', 'Age > 50', 1),
  ...
  },
  {
    ...
    inputs: [
      ...
      ], undefined, 'Modified MRC dyspnea scale. Pick the worst applicable grade, then the BODE point bin (grades 0 and 1 both score 0 BODE points).'),
