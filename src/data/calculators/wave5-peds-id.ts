...
export const wave5PedsIdCalcs: Calculator[] = [
  // ─── 1. Full PEWS (multi-domain) ───────────────────────────────────────────
  {
    ...
    inputs: [
      selectInput('behavior', 'Behavior / consciousness', [
        { label: 'Playing / appropriate (0)', value: 0, description: 'Alert, interacting, behavior appropriate for age' },
        { label: 'Sleeping (1)', value: 1, description: 'Sleeping but arousable to expected stimuli, then appropriate' },
        { label: 'Irritable (2)', value: 2, description: 'Irritable, difficult to console' },
        { label: 'Lethargic / confused / reduced pain response (3)', value: 3, description: 'Lethargic, confused, or reduced response to pain — urgent review' },
      ], 1, 'Score the current behavior. Sleeping that arouses appropriately is 1, not 3. Reduced pain response is 3.'),
      ...
      selectInput('resp', 'Respiratory rate / effort', [
        ...
        { label: 'RR moderately ↑, recession, or grunting (2)', value: 2, description: 'RR >20 above age-normal max, recession, or grunting' },
        { label: 'Severe distress, apnea risk, or RR markedly low with distress (3)', value: 3, description: 'RR ≥5 below age-normal min with distress, severe work of breathing, or apnea risk' },
      ], 2, 'Score RR and effort against PALS awake resting RR: neonate 30–60; infant 30–53; toddler 22–37; preschool 20–28; school-age 18–25; adolescent 12–20. Count for a full minute at rest. Monaghan-style offsets: 0 = within age normal, no recession; 1 = >10 above normal or mild accessory; 2 = >20 above, recession, or grunting; 3 = ≥5 below normal with distress or apnea risk.'),
      selectInput('oxygen', 'Oxygen requirement', [
        { label: 'Room air (0)', value: 0, description: 'No supplemental oxygen' },
        { label: 'Any low-flow O₂ / ≤30% (1)', value: 1, description: 'Nasal cannula or FiO₂ ≤0.30' },
        { label: 'Higher O₂ / ~40% or high-flow (2)', value: 2, description: 'FiO₂ around 0.40, or high-flow nasal cannula' },
        { label: '≥50% / NIV / invasive ventilation (3)', value: 3, description: 'FiO₂ ≥0.50, NIV (CPAP/BiPAP), or invasive ventilation' },
      ], 1, 'Oxygen requirement: room air 0, any low-flow O₂ or ≤30% 1, around 40% or high-flow 2, and ≥50%/NIV/invasive ventilation 3 points.'),
      selectInput('spo2', 'SpO₂ (on current support)', [
        { label: '≥94% or at known baseline (0)', value: 0, description: 'SpO₂ ≥94%, or at the child’s known cyanotic-heart baseline' },
        { label: '91–93% (1)', value: 1, description: '91–93% on current support (unless that is the known baseline)' },
        { label: '≤90% (2)', value: 2, description: '≤90% on current support (unless known baseline)' },
      ], 1, 'SpO₂ on the current support: 94% or above (or the child\'s known baseline) 0, 91–93% 1, and 90% or below 2 points.'),
      selectInput('temp', 'Temperature', [
        { label: '36.1–38.0 °C (0)', value: 0, description: '36.1–38.0 °C (97.0–100.4 °F)' },
        { label: '35.1–36.0 or 38.1–39.0 (1)', value: 1, description: '35.1–36.0 °C or 38.1–39.0 °C' },
        { label: '≤35.0 or >39.0 (2)', value: 2, description: '≤35.0 °C or >39.0 °C (95.0 / 102.2 °F)' },
      ], 1, 'Temperature: 36.1–38.0°C scores 0, 35.1–36.0 or 38.1–39.0 scores 1, and ≤35.0 or >39.0 scores 2 points.'),
      yesNo('nurseConcern', 'Staff / caregiver concern (local bonus +1)', 1,
        'Nurse or caregiver is worried the child is deteriorating, even if other domains are not at maximum. Local PEWS charts vary — this +1 is educational.', true),
    ],
    calculate(values) {
      const score =
        num(values.behavior) +
  ...
  // ─── 2. PALS normal HR ─────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'awake', 'Awake/resting uses the full expected range; sleeping shifts the expected rate to the lower end, so a value that looks bradycardic awake may be normal asleep.'),
    ],
    calculate(values) {
  ...
  // ─── 3. PALS normal RR ─────────────────────────────────────────────────────
  {
    ...
    inputs: [
      selectInput('ageBand', 'Age band', [
        ...
        { label: 'School age (6–11 years)', value: 'school' },
        { label: 'Adolescent (12–15 years)', value: 'teen' },
      ], 'toddler', 'Age band sets the expected range: neonate 30–60, infant 30–53, toddler 22–37, preschool 20–28, school age 18–25, adolescent 12–20 breaths/min.'),
  ...
  // ─── 4. PALS minimum SBP ───────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'child', 'Minimum systolic BP: term neonate 60, infant 70, 1–10 years 70 + 2 × age, and over 10 years 90 mmHg. Hypotension is a late sign of pediatric shock.'),
  ...
  // ─── 5. ETT depth ──────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'oral', 'Oral depth is the standard formula; nasal insertion is roughly 2–3 cm deeper. Confirm with auscultation, ETCO₂, and imaging where available.'),
    ],
    calculate(values) {
  ...
  // ─── 6. Defibrillation dose ────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('weight', 'Weight', { unit: 'kg', unitKind: 'weight', min: 2, max: 100, step: 0.1, exampleValue: 15, helpText: 'Weight in kg for J/kg dosing (select lb if needed). Verify the defibrillator\'s maximum energy and follow the local PALS protocol.' }),
      ...
      ], 'first', 'First shock 2 J/kg; subsequent shocks 4 J/kg (some protocols cap at 10 J/kg or the adult maximum). Choose the branch that matches the shock you are delivering.'),
    ],
    calculate(values) {
  ...
  // ─── 7. Epinephrine arrest dose ────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('weight', 'Weight', { unit: 'kg', unitKind: 'weight', min: 1, max: 100, step: 0.1, exampleValue: 15, helpText: 'Weight in kg for 0.01 mg/kg IV/IO (0.1 mL/kg of 1:10,000); the single-dose cap is 1 mg (10 mL of 1:10,000).' }),
      ...
      ], 'iv', 'IV/IO uses 0.1 mg/mL (1:10,000) and is the current standard; the endotracheal route is deprecated in current AHA PALS arrest algorithms.'),
    ],
    calculate(values) {
      const w = num(values.weight, 15);
  ...
  // ─── 8. Fluid bolus ───────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('weight', 'Weight', { unit: 'kg', unitKind: 'weight', min: 1, max: 100, step: 0.1, exampleValue: 15, helpText: 'Weight in kg for the mL/kg bolus volume; select lb if the chart is imperial. Reassess perfusion after each bolus rather than stacking volumes.' }),
      ...
      ], 20, '20 mL/kg is the typical initial bolus outside cardiac disease; 10 mL/kg is used for cardiac or neonatal patients and when fluid overload is a concern.'),
      ...
      ], 'crystalloid', 'Isotonic crystalloid (NS or LR) is standard; the blood-product option is a label only and has separate indications for hemorrhagic shock.'),
    ],
    calculate(values) {
      const w = num(values.weight, 15);
  ...
  // ─── 9. kcal needs ─────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'toddler', 'Age band selects the kcal/kg/day teaching range: preterm 110–130+, term infant 100–120, older infant 90–100, 1–3 years 80–100, and declining further with age.'),
      numberInput('weight', 'Weight', { unit: 'kg', unitKind: 'weight', min: 1, max: 120, step: 0.1, exampleValue: 12, helpText: 'Weight in kg to convert the kcal/kg range into a daily target; select lb if needed. Not a substitute for dietitian assessment or indirect calorimetry.' }),
      selectInput('stress', 'Illness / activity multiplier', [
        ...
        { label: 'Moderate illness (×1.4)', value: 1.4, description: 'Significant infection, moderate trauma, or inflammatory disease requiring extra energy; not major burn/ICU-level stress.' },
        { label: 'Severe stress / burn / major surgery (×1.6)', value: 1.6, description: 'Major surgery, extensive burns, severe sepsis/trauma. Prefer dietitian assessment or indirect calorimetry — this multiplier is only a starting estimate.' },
      ], 1, 'Educational stress factors applied to age-band kcal/kg. Critical illness may need lower early targets then titration — do not blindly multiply in the ICU.'),
    ],
    calculate(values) {
  ...
  // ─── 10. Failure to thrive ─────────────────────────────────────────────────
  {
    ...
    inputs: [
      selectInput('waz', 'Weight-for-age z-score band', [
        { label: 'z > −1 (normal band)', value: 0, description: 'Above about the 16th percentile (WHO/CDC WFA). Not underweight on this axis.' },
        { label: 'z −1 to −2 (mild concern)', value: 1, description: 'Roughly 3rd–16th percentile. WHO underweight starts at z < −2; this band is a watch zone, especially with poor velocity.' },
        { label: 'z −2 to −3 (moderate underweight)', value: 2, description: 'WHO moderate underweight (below ~3rd percentile). Plot on WHO (0–2 y) or CDC (≥2 y) charts — this tool does not compute z from raw weight.' },
        { label: 'z < −3 (severe underweight)', value: 3, description: 'WHO severe underweight. Urgent nutritional and medical evaluation.' },
      ], 1, 'Enter the weight-for-age z already obtained from a WHO/CDC chart or calculator. Do not estimate z by eye.'),
      yesNo('crossing', 'Crossed ≥2 major percentile lines downward', 1,
        'Major lines typically 95th, 90th, 75th, 50th, 25th, 10th, 5th. Downward crossing of two or more over time (not a single measurement).', false),
      yesNo('weightLtLength', 'Weight-for-length / BMI z < −2', 1,
        'WHO wasting: weight-for-length (0–2 y) or BMI-for-age (≥2 y) z < −2 (moderate) or < −3 (severe). From the length/height-based chart, not WFA.', false),
      yesNo('poorGain', 'Documented poor weight velocity / gain', 1,
        'Examples: weight gain well below expected for age (term infant often <20 g/day over weeks, or flattening on a WHO velocity chart), or down-crossing not already captured as ≥2 major percentile lines. Use growth-chart / WHO velocity, not a single weight.', true),
      yesNo('redFlags', 'Red flags (vomiting, diarrhea, neglect concern, chronic disease signs)', 2,
        'Persistent vomiting or diarrhea, signs of neglect/food insecurity, or exam findings of chronic disease. Adds 2 points.', false),
      numberInput('ageMonths', 'Age', { unit: 'months', min: 0, max: 216, exampleValue: 12, helpText: 'Age in months (0–216) tells the tool which WHO/CDC growth standard applies; z-score bands are read against the age- and sex-matched chart, so enter the exact age.' }),
    ],
    calculate(values) {
      const score =
        num(values.waz) +
  ...
  // ─── 11. Jaundice / Bhutani ────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('ageHours', 'Age', { unit: 'hours', min: 12, max: 144, exampleValue: 48, helpText: 'Postnatal age in hours (12–144) is the x-axis of the Bhutani nomogram; the risk zone depends as much on age in hours as on the TSB value.' }),
      numberInput('tsb', 'Total serum bilirubin', { unit: 'mg/dL', min: 1, max: 30, step: 0.1, exampleValue: 10, helpText: 'Total serum bilirubin in mg/dL from the same draw; the tool compares it against the hour-specific risk-zone floors rather than a single cutoff.' }),
      ...
      ], 'lowRiskPop', 'Neurotoxicity / higher-risk factors besides GA (AAP 2022 Table 2): albumin <3.0 g/dL; isoimmune hemolytic disease (DAT+), G6PD, or other hemolysis; sepsis; significant clinical instability in the previous 24 h. Bhutani zones are for ≥35-week newborns; hour of age is postnatal hours, not days.'),
    ],
    calculate(values) {
  ...
  // ─── 12. Phototherapy threshold ────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      numberInput('ageHours', 'Age', { unit: 'hours', min: 6, max: 336, exampleValue: 48, helpText: 'Age in hours (6–336) drives the AAP 2022 phototherapy threshold curves, which rise as the newborn gets older.' }),
      numberInput('tsb', 'Total serum bilirubin', { unit: 'mg/dL', min: 1, max: 35, step: 0.1, exampleValue: 14, helpText: 'Total serum bilirubin in mg/dL; the threshold also shifts with gestational age and neurotoxicity risk factors, so confirm with the official AAP tables.' }),
  ...
  // ─── 13. Exchange transfusion threshold ────────────────────────────────────
  {
    ...
    inputs: [
      ...
      numberInput('ageHours', 'Age', { unit: 'hours', min: 6, max: 336, exampleValue: 48, helpText: 'Age in hours (6–336) sets the exchange threshold curve; first-24-hour values can rise rapidly, so use the most recent TSB.' }),
      numberInput('tsb', 'Total serum bilirubin', { unit: 'mg/dL', min: 1, max: 45, step: 0.1, exampleValue: 22, helpText: 'Total serum bilirubin in mg/dL. Acute bilirubin encephalopathy signs prompt exchange regardless of the number, and the official AAP tables remain the reference.' }),
      selectInput('risk', 'Neurotoxicity risk factors other than GA', [
  ...
  // ─── 14. Croup dex dose ────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('weight', 'Weight', { unit: 'kg', unitKind: 'weight', min: 3, max: 80, step: 0.1, exampleValue: 12, helpText: 'Weight in kg for the mg/kg dose; select lb if needed. The maximum single dose is commonly 10–16 mg depending on protocol.' }),
      ...
      ], 0.15, '0.15–0.6 mg/kg once is the accepted range; 0.15 mg/kg (or 0.6 mg/kg) both work, and higher doses do not add benefit. Convert to mL using the concentration on hand.'),
  ...
  // ─── 15. Pulmonary score ───────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'young', 'Use the selected age band with the Smith 2002 RR table. Count RR for 1 full minute at rest.'),
      selectInput('rr', 'Respiratory rate points', [
        { label: 'Normal for age (0)', value: 0, description: '<6 y: ≤30 bpm; ≥6 y: ≤20 bpm' },
        { label: 'Mildly elevated (1)', value: 1, description: '<6 y: 31–45 bpm; ≥6 y: 21–35 bpm' },
        { label: 'Moderately elevated (2)', value: 2, description: '<6 y: 46–60 bpm; ≥6 y: 36–50 bpm' },
        { label: 'Severely elevated (3)', value: 3, description: '<6 y: >60 bpm; ≥6 y: >50 bpm' },
      ], 2, 'Smith 2002 age-specific RR (count 1 full minute at rest): <6 y — 0 ≤30, 1 = 31–45, 2 = 46–60, 3 >60; ≥6 y — 0 ≤20, 1 = 21–35, 2 = 36–50, 3 >50.'),
      ...
      ], 1, 'Wheezing: none 0, end-expiratory only 1, expiratory throughout 2, and inspiratory plus expiratory 3 points — the heaviest item in the score.'),
      selectInput('accessory', 'Accessory muscle use', [
        ...
        { label: 'Mild SCM (1)', value: 1, description: 'Mild increase in sternocleidomastoid activity' },
        { label: 'Increased SCM (2)', value: 2, description: 'Increased sternocleidomastoid activity' },
        { label: 'Maximal SCM (3)', value: 3, description: 'Maximal sternocleidomastoid activity' },
      ], 1, 'Official pulmonary score accessory domain is sternocleidomastoid (SCM) activity, not generic work of breathing.'),
      numberInput('spo2', 'SpO₂ on room air (optional context)', {
        ...
        exampleValue: 96,
        required: false,
        helpText: 'SpO₂ on room air in % for context only; oxygen saturation is not part of the pulmonary score itself but guides severity labeling alongside it.',
      }),
    ],
    calculate(values) {
  ...
  // ─── 16. Scarlet fever helper ──────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('pharyngitis', 'Acute pharyngitis / tonsillitis', 1,
        'Sore throat with tonsillar erythema/exudate or palatal petechiae in this illness.', true),
      yesNo('fever', 'Fever', 1,
        'Documented temperature ≥38.0 °C (100.4 °F) or a clear history of fever with this illness.', true),
      yesNo('sandpaper', 'Diffuse sandpaper-like erythematous rash', 2,
        'Fine papular, sandpaper-texture erythema, often starting in flexures then spreading; spares perioral area (perioral pallor).', true),
      yesNo('pastia', 'Pastia lines (linear petechiae in creases)', 1, 'Pastia lines (linear petechiae in the antecubital or axillary creases) add 1 point and support the toxin-mediated rash.', false),
      yesNo('strawberry', 'Strawberry tongue / flushed face with perioral pallor', 1, 'Strawberry tongue or flushed cheeks with circumoral pallor adds 1 point.', true),
      yesNo('desquamation', 'Peeling (often later, hands/feet)', 1, 'Peeling, often later and involving the hands and feet, adds 1 point but appears after the acute phase.', false),
      yesNo('exposure', 'Known strep exposure or positive RADT/culture', 2, 'Known strep exposure or a positive rapid antigen test/culture adds 2 points — the highest-weighted item.', false),
      yesNo('viralFeatures', 'Prominent viral features (cough, rhinorrhea, conjunctivitis) arguing against strep', -2, 'Prominent viral features (cough, rhinorrhea, conjunctivitis) subtract 2 points because they argue against group A strep.', false),
    ],
    calculate(values) {
      let score = 0;
  ...
  // ─── 17. PIMS / MIS-C ──────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('ageOk', 'Age <21 years (CDC) / child-adolescent', 1, 'Age under 21 years (the CDC case definition) adds 1 point; the WHO/PIMS-TS definition uses a pediatric age range instead.', true),
      yesNo('fever', 'Fever ≥3 days (or ≥24 h if in shock — follow local def.)', 1, 'Fever for 3 days or longer (or ≥24 hours if in shock, per some definitions) adds 1 point.', true),
      yesNo('inflammation', 'Laboratory inflammation (↑CRP/ESR, neutrophilia, lymphopenia, etc.)', 1,
        'CDC MIS-C-style: CRP ≥3.0 mg/dL (30 mg/L) plus at least one of lymphopenia, thrombocytopenia, hyponatremia, hypoalbuminemia, or neutrophilia; ESR ≥40 mm/h supportive. WHO: elevated CRP, ESR, or procalcitonin.', true),
      yesNo('multiSystem', '≥2 organ systems involved (cardio, mucocutaneous, GI, renal, neuro, heme, resp)', 2,
        'Examples: cardiac (shock, dysfunction, arrhythmia, coronary); mucocutaneous (rash, conjunctivitis, strawberry tongue, swollen hands/feet); GI (pain, vomiting, diarrhea); renal (AKI); neuro (altered mental status, headache, meningism); heme (coagulopathy, thrombocytopenia); respiratory (need for support).', true),
      yesNo('noAlt', 'No plausible alternative diagnosis fully explaining illness', 1, 'No plausible alternative diagnosis fully explaining the illness adds 1 point; bacterial sepsis and Kawasaki disease are the usual confounders.', true),
      yesNo('covidLink', 'Evidence of SARS-CoV-2 (PCR/Ag/serology) or close exposure', 2, 'Evidence of SARS-CoV-2 (PCR, antigen, or serology) or a close exposure adds 2 points — the highest-weighted item.', true),
      yesNo('shockKd', 'Shock, myocardial dysfunction, or Kawasaki-like features', 1, 'Shock, myocardial dysfunction, or Kawasaki-like features adds 1 point and often drives ICU-level care.', false),
      yesNo('severe', 'ICU-level illness / vasoactive need / severe organ injury', 1, 'ICU-level illness, vasoactive requirement, or severe organ injury adds 1 point; involve ID/rheumatology/cardiology early.', false),
    ],
    calculate(values) {
      const score =
  ...
  // ─── 18. Orbital cellulitis red flags ──────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('painEom', 'Pain with eye movements', 2,
        'Pain on extraocular movement — a post-septal/orbital red flag (not mere lid tenderness).', true),
      yesNo('ophthalmoplegia', 'Limited extraocular movements / diplopia', 2,
        'Restricted ductions or binocular diplopia.', true),
      yesNo('proptosis', 'Proptosis', 2,
        'Forward displacement of the globe; compare from above (chin-up view).', true),
      yesNo('vision', 'Vision change / relative afferent pupillary defect concern', 2,
        'Reduced acuity, color desaturation, or RAPD — urgent ophthalmology.', false),
      yesNo('chemosis', 'Chemosis / severe conjunctival swelling', 1, 'Chemosis or severe conjunctival swelling adds 1 point and suggests post-septal involvement.', false),
      yesNo('feverToxic', 'Fever or toxic appearance', 1, 'Fever or a toxic appearance adds 1 point and lowers the threshold for imaging.', true),
      yesNo('sinus', 'Recent/concurrent sinusitis', 1, 'Recent or concurrent sinusitis adds 1 point — the most common source of orbital cellulitis.', true),
      yesNo('bilateral', 'Bilateral findings (consider cavernous sinus thrombosis)', 1, 'Bilateral findings add 1 point and raise concern for cavernous sinus thrombosis.', false),
      yesNo('neuro', 'Headache, vomiting, altered mentation, meningism', 2, 'Headache, vomiting, altered mentation, or meningism adds 2 points and mandates urgent imaging and specialty involvement.', false),
    ],
    calculate(values) {
      const score =
  ...
  // ─── 19. GMSPS ─────────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('hypotension', 'BP <75 mmHg systolic if <4 y, or <85 if ≥4 y', 3,
        'GMSPS hypotension: SBP <75 mmHg if age <4 years, or SBP <85 mmHg if age ≥4 years.', false),
  ...
  // ─── 20. Petechiae risk (NICE-style) ───────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('purpura', 'Purpura / non-blanching rash spreading or large purpuric lesions', 3,
        'Spreading non-blanching rash or large purpuric lesions (not a few stable petechiae).', false),
      yesNo('ill', 'Appears ill / toxic / lethargic', 2,
        'Toxic, lethargic, or significantly ill appearance — NICE red feature.', false),
      yesNo('capRefill', 'Capillary refill ≥3 s or cold extremities / shock signs', 3,
        'CRT ≥3 seconds, or cold extremities, or other shock signs.', false),
      yesNo('neckStiff', 'Neck stiffness, bulging fontanelle, or photophobia', 2, 'Neck stiffness, a bulging fontanelle, or photophobia adds 2 points and is a red feature.', false),
      yesNo('seizure', 'Seizure or focal neuro signs', 2, 'Seizure or focal neurologic signs add 2 points and indicate immediate management.', false),
      yesNo('resp', 'Grunting, severe tachypnea for age, or SpO₂ <92% in air', 2,
        'NICE under-5s red: SpO₂ <92% in air, grunting, moderate/severe chest indrawing, tachypnea ≥60 if <6 months.', false),
      yesNo('ageYoung', 'Age <3 months with fever', 2, 'Age under 3 months with fever adds 2 points; this group is investigated aggressively regardless of appearance.', true),
      yesNo('localized', 'Petechiae only above nipple line after coughing/vomiting (mechanical)', -1, 'Petechiae confined above the nipple line after coughing or vomiting (mechanical pattern) subtracts 1 point.', false),
      yesNo('wellAppearing', 'Well-appearing, playful, normal vitals', -1, 'A well-appearing, playful child with normal vitals subtracts 1 point, though local protocols may still require investigation.', true),
    ],
    calculate(values) {
      let score = 0;
  ...
  // ─── 21. UTI peds ──────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'infant', 'Age group: 2 months or younger, 2–24 months, or older than 24 months. Specimen collection method and testing thresholds differ by stratum.'),
      selectInput('sexCirc', 'Sex / circumcision', [
        { label: 'Female', value: 'F' },
        { label: 'Male uncircumcised', value: 'Mu' },
        { label: 'Male circumcised', value: 'Mc' },
      ], 'F', 'Female, male uncircumcised, or male circumcised; UTI risk in the first year is substantially higher in uncircumcised boys than circumcised boys.'),
      yesNo('fever', 'Fever (especially ≥39 °C or ≥24–48 h)', 1, 'Fever, especially 39°C or higher or lasting 24–48 hours, adds 1 point.', true),
      yesNo('noSource', 'No alternative fever source', 1, 'No alternative fever source adds 1 point; a clear viral focus lowers the likelihood of UTI.', true),
      yesNo('historyUti', 'Prior UTI or known urinary tract anomaly', 2, 'Prior UTI or a known urinary tract anomaly adds 2 points.', false),
      yesNo('urinarySx', 'Dysuria, frequency, urgency, new incontinence (if verbal)', 2, 'Dysuria, frequency, urgency, or new incontinence in a verbal child adds 2 points.', false),
      yesNo('abdFlank', 'Abdominal or flank pain / suprapubic tenderness', 1, 'Abdominal or flank pain, or suprapubic tenderness, adds 1 point.', false),
      yesNo('malodorous', 'Malodorous urine (weak alone)', 1, 'Malodorous urine adds 1 point but is weak evidence on its own.', false),
      yesNo('ill', 'Ill appearance / vomiting / poor feeding', 1, 'Ill appearance, vomiting, or poor feeding adds 1 point; this shifts the plan toward bloodwork and admission.', false),
    ],
    calculate(values) {
      let score = 0;
  ...
  // ─── 22. Pneumonia peds severity ───────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('hypoxia', 'SpO₂ <92% (or local threshold) on air', 2, 'SpO₂ below 92% on air (or below the local threshold) adds 2 points and usually requires admission and oxygen.', true),
      yesNo('distress', 'Moderate–severe work of breathing / grunting / head bobbing', 2, 'Moderate-to-severe work of breathing, grunting, or head bobbing adds 2 points.', true),
      yesNo('rrHigh', 'RR substantially above age normal (BTS: >70/min if infant, >50/min if older child)', 1,
        'BTS-style severe-CAP tachypnea: >70 breaths/min in infants, >50/min in older children. Alternatively, ~20–30 above PALS age-max RR (infant 30–53, toddler 22–37, preschool 20–28, school 18–25).', false),
      yesNo('poorIntake', 'Not feeding / dehydrated / unable to take orals', 2, 'Not feeding, dehydration, or inability to take oral medication adds 2 points.', false),
      yesNo('toxic', 'Toxic appearance, altered mentation, or apnea', 3, 'Toxic appearance, altered mentation, or apnea adds 3 points — the highest-weighted item.', false),
      yesNo('effusion', 'Significant effusion / empyema suspicion', 2, 'Significant effusion or suspected empyema adds 2 points and prompts imaging and drainage consideration.', false),
      yesNo('infant', 'Age <3 months (higher risk band)', 1,
        'IDSA/PIDS and BTS treat very young infants as higher-risk CAP. This item uses <3 months; some pathways extend concern through <6 months — apply local policy.', false),
      yesNo('comorbid', 'Significant comorbidity (cardiac, chronic lung, immuno)', 1, 'Significant cardiac, chronic lung, or immunocompromising comorbidity adds 1 point.', false),
    ],
    calculate(values) {
      const score =
  ...
  // ─── 23. WHO pneumonia ─────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'infant', 'Age band sets the fast-breathing threshold: 2–11 months is ≥50 breaths/min and 12–59 months is ≥40 breaths/min (both counted over a full minute).'),
      ...
      yesNo('chestIndrawing', 'Chest indrawing', null,
        'Inward movement of the lower chest wall on inspiration while the child is calm (not isolated intercostal recession or crying).', true),
      yesNo('danger', 'General danger sign (not able to drink, persistent vomiting, convulsions, lethargy/unconscious, stridor in calm child)', null,
        'Any one general danger sign defines severe pneumonia — this classification is not additive.', false),
      yesNo('spo2Low', 'SpO₂ <90% (if pulse oximetry available)', null,
        'Hypoxaemia outranks the fast-breathing/indrawing class and defines severe pneumonia.', false),
      yesNo('malnutrition', 'Severe acute malnutrition (context)', null,
        'Severe acute malnutrition is a severe-disease flag in this helper, not an additive point.', false),
    ],
    calculate(values) {
  ...
  // ─── 24. Malaria severity ──────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('impairedConscious', 'Impaired consciousness / coma (GCS <11 or Blantyre <3)', 1,
        'WHO: GCS <11 (or Blantyre coma score <3 in children).', false),
      yesNo('prostration', 'Prostration (unable to sit/stand/walk unassisted)', 1, 'Prostration — unable to sit, stand, or walk unassisted for the age — counts as a WHO severe feature.', true),
      yesNo('multipleSeizure', 'Multiple convulsions (>2 in 24 h)', 1, 'More than two convulsions in 24 hours counts as a severe feature.', false),
      yesNo('acidosis', 'Acidosis / acidotic breathing (base deficit >8, HCO3 <15, or lactate ≥5)', 1,
        'WHO: base deficit >8 mEq/L, plasma bicarbonate <15 mmol/L, or venous lactate ≥5 mmol/L; or deep (Kussmaul) breathing.', true),
      yesNo('hypoglycemia', 'Hypoglycemia (glucose <40–45 mg/dL / <2.2–2.5 mmol/L)', 1, 'Glucose below 40–45 mg/dL (2.2–2.5 mmol/L) counts as a severe feature and needs immediate dextrose.', false),
      yesNo('anemia', 'Severe anemia (Hb <5 g/dL if <12 y; <7 g/dL if adult) with parasites', 1,
        'WHO: Hb <5 g/dL (child <12 y) or <7 g/dL (adult) with parasite density >10,000/µL.', true),
      yesNo('renal', 'Acute kidney injury (Cr >3 mg/dL or urea >20 mmol/L) / oliguria', 1,
        'WHO: serum creatinine >3 mg/dL (265 µmol/L) or urea >20 mmol/L, or oliguria.', false),
      yesNo('jaundice', 'Jaundice (bilirubin >3 mg/dL) with parasites >100,000/µL', 1,
        'WHO: plasma or serum bilirubin >3 mg/dL (50 µmol/L) with parasite density >100,000/µL.', false),
      yesNo('pulmonary', 'Pulmonary edema / ARDS (radiographic edema or SpO2 <92% with RR >30)', 1,
        'WHO: radiographic pulmonary edema, or SpO2 <92% on room air with respiratory rate >30/min.', false),
      yesNo('bleeding', 'Significant bleeding / DIC', 1, 'Significant bleeding or DIC counts as a severe feature.', false),
      yesNo('shock', 'Shock (CRT ≥3 s or SBP <80 adult / <70 child)', 1,
        'WHO: compensated shock = capillary refill ≥3 s or temperature gradient; hypotensive shock = SBP <80 mmHg adult or <70 mmHg child.', false),
      yesNo('hyperparasitemia', 'Hyperparasitemia (Pf >10%; CDC often >5% in non-immune travelers)', 1,
        'WHO: P. falciparum parasitemia >10%. CDC often uses >5% in non-immune travelers. Threshold varies by epidemiology.', false),
    ],
    calculate(values) {
      const keys = [
        'impairedConscious',
  ...
  // ─── 25. Dengue warning signs ──────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('abdPain', 'Abdominal pain or tenderness', 1,
        'Persistent abdominal pain or tenderness, not a single brief cramp.', true),
      yesNo('persistentVomiting', 'Persistent vomiting', 1,
        'Repeated vomiting that prevents adequate oral intake (often ≥3 episodes in 1 hour).', true),
      yesNo('fluidAccum', 'Clinical fluid accumulation (ascites/effusion/edema)', 1,
        'Pleural effusion, ascites, or edema on exam or imaging — a WHO warning sign of plasma leakage.', false),
      yesNo('mucosalBleed', 'Mucosal bleeding', 1,
        'Epistaxis, gum bleeding, hematemesis, melena, or menorrhagia — not isolated petechiae.', false),
      yesNo('lethargy', 'Lethargy / restlessness', 1,
        'Change in alertness: lethargy or restlessness (WHO warning), not ordinary tiredness.', false),
      yesNo('liver', 'Liver enlargement >2 cm', 1,
        'Liver edge >2 cm below the costal margin (WHO warning sign).', false),
      yesNo('hctRising', 'Lab: rising hematocrit with rapid platelet fall', 1,
        'Hematocrit rising concurrent with a rapid drop in platelet count (plasma leakage).', true),
      yesNo('shock', 'Shock (pulse pressure ≤20 mmHg, hypotension, or cold clammy skin / poor perfusion)', 3,
        'WHO compensated shock includes pulse pressure ≤20 mmHg; hypotensive shock is unrecordable/low BP with perfusion failure.', false),
      yesNo('severeBleed', 'Severe bleeding', 3, 'Severe bleeding adds 3 points and places the patient in the severe-dengue pathway with urgent resuscitation.', false),
      yesNo('organ', 'Severe organ impairment (AST/ALT massive, impaired consciousness, myocarditis, etc.)', 3, 'Severe organ impairment (massively elevated transaminases, impaired consciousness, myocarditis) adds 3 points and also defines severe dengue.', false),
    ],
    calculate(values) {
      const warnings = [
        'abdPain',
