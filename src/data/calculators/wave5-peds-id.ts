...
export const wave5PedsIdCalcs: Calculator[] = [
  // ─── 1. Full PEWS (multi-domain) ───────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], undefined, 'Score the current behavior. Sleeping that arouses appropriately is 1, not 3. Reduced pain response is 3.'),
      ...
      ], undefined, 'Score color, capillary refill, and HR against age-normal awake resting HR (PALS): neonate 100–205; infant 100–180; toddler 98–140; preschool 80–120; school-age 75–118; adolescent 60–100. “+20 / +30 above normal” is vs the upper end of that age band; bradycardia is below the lower end with poor perfusion.'),
      ...
      ], undefined, 'Score RR and effort against PALS awake resting RR: neonate 30–60; infant 30–53; toddler 22–37; preschool 20–28; school-age 18–25; adolescent 12–20. Count for a full minute at rest. Monaghan-style offsets: 0 = within age normal, no recession; 1 = >10 above normal or mild accessory; 2 = >20 above, recession, or grunting; 3 = ≥5 below normal with distress or apnea risk.'),
      ...
      yesNo('nurseConcern', 'Staff / caregiver concern (local bonus +1)', 1,
        'Nurse or caregiver is worried the child is deteriorating, even if other domains are not at maximum. Local PEWS charts vary — this +1 is educational.'),
    ],
    calculate(values) {
      const score =
        num(values.behavior) +
  ...
  // ─── 9. kcal needs ─────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], undefined, 'Educational stress factors applied to age-band kcal/kg. Critical illness may need lower early targets then titration — do not blindly multiply in the ICU.'),
    ],
    calculate(values) {
  ...
  // ─── 10. Failure to thrive ─────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], undefined, 'Enter the weight-for-age z already obtained from a WHO/CDC chart or calculator. Do not estimate z by eye.'),
      yesNo('crossing', 'Crossed ≥2 major percentile lines downward', 1,
        'Major lines typically 95th, 90th, 75th, 50th, 25th, 10th, 5th. Downward crossing of two or more over time (not a single measurement).'),
      yesNo('weightLtLength', 'Weight-for-length / BMI z < −2', 1,
        'WHO wasting: weight-for-length (0–2 y) or BMI-for-age (≥2 y) z < −2 (moderate) or < −3 (severe). From the length/height-based chart, not WFA.'),
  ...
  // ─── 11. Jaundice / Bhutani ────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], undefined, 'Neurotoxicity / higher-risk factors besides GA (AAP 2022 Table 2): albumin <3.0 g/dL; isoimmune hemolytic disease (DAT+), G6PD, or other hemolysis; sepsis; significant clinical instability in the previous 24 h. Bhutani zones are for ≥35-week newborns; hour of age is postnatal hours, not days.'),
    ],
    calculate(values) {
  ...
  // ─── 15. Pulmonary score ───────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], undefined, 'Use the selected age band with the Smith 2002 RR table. Count RR for 1 full minute at rest.'),
      ...
      ], undefined, 'Official pulmonary score accessory domain is sternocleidomastoid (SCM) activity, not generic work of breathing.'),
  ...
  // ─── 16. Scarlet fever helper ──────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('pharyngitis', 'Acute pharyngitis / tonsillitis', 1,
        'Sore throat with tonsillar erythema/exudate or palatal petechiae in this illness.'),
      yesNo('fever', 'Fever', 1,
        'Documented temperature ≥38.0 °C (100.4 °F) or a clear history of fever with this illness.'),
      yesNo('sandpaper', 'Diffuse sandpaper-like erythematous rash', 2,
        'Fine papular, sandpaper-texture erythema, often starting in flexures then spreading; spares perioral area (perioral pallor).'),
  ...
  // ─── 17. PIMS / MIS-C ──────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('inflammation', 'Laboratory inflammation (↑CRP/ESR, neutrophilia, lymphopenia, etc.)', 1,
        'CDC MIS-C-style: CRP ≥3.0 mg/dL (30 mg/L) plus at least one of lymphopenia, thrombocytopenia, hyponatremia, hypoalbuminemia, or neutrophilia; ESR ≥40 mm/h supportive. WHO: elevated CRP, ESR, or procalcitonin.'),
      yesNo('multiSystem', '≥2 organ systems involved (cardio, mucocutaneous, GI, renal, neuro, heme, resp)', 2,
        'Examples: cardiac (shock, dysfunction, arrhythmia, coronary); mucocutaneous (rash, conjunctivitis, strawberry tongue, swollen hands/feet); GI (pain, vomiting, diarrhea); renal (AKI); neuro (altered mental status, headache, meningism); heme (coagulopathy, thrombocytopenia); respiratory (need for support).'),
  ...
  // ─── 18. Orbital cellulitis red flags ──────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('painEom', 'Pain with eye movements', 2,
        'Pain on extraocular movement — a post-septal/orbital red flag (not mere lid tenderness).'),
      yesNo('ophthalmoplegia', 'Limited extraocular movements / diplopia', 2,
        'Restricted ductions or binocular diplopia.'),
      yesNo('proptosis', 'Proptosis', 2,
        'Forward displacement of the globe; compare from above (chin-up view).'),
      yesNo('vision', 'Vision change / relative afferent pupillary defect concern', 2,
        'Reduced acuity, color desaturation, or RAPD — urgent ophthalmology.'),
  ...
  // ─── 19. GMSPS ─────────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('hypotension', 'BP <75 mmHg systolic if <4 y, or <85 if ≥4 y', 3,
        'GMSPS hypotension: SBP <75 mmHg if age <4 years, or SBP <85 mmHg if age ≥4 years.'),
      yesNo('skinTemp', 'Skin–rectal temperature difference >3 °C', 3,
        'Peripheral skin temperature more than 3 °C cooler than rectal (core) temperature.'),
      yesNo('coma', 'Modified coma scale <8 / deeply impaired consciousness', 3,
        'Use pediatric-modified GCS (E4 V5 M6, max 15). Score +3 if total <8. Infant verbal: 5 coos/babbles, 4 irritable cry, 3 cries to pain, 2 moans to pain, 1 none. If intubated, use best pre-intubation exam.'),
  ...
  // ─── 20. Petechiae risk (NICE-style) ───────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('purpura', 'Purpura / non-blanching rash spreading or large purpuric lesions', 3,
        'Spreading non-blanching rash or large purpuric lesions (not a few stable petechiae).'),
      yesNo('ill', 'Appears ill / toxic / lethargic', 2,
        'Toxic, lethargic, or significantly ill appearance — NICE red feature.'),
      yesNo('capRefill', 'Capillary refill ≥3 s or cold extremities / shock signs', 3,
        'CRT ≥3 seconds, or cold extremities, or other shock signs.'),
      ...
      yesNo('resp', 'Grunting, severe tachypnea for age, or SpO₂ <92% in air', 2,
        'NICE under-5s red: SpO₂ <92% in air, grunting, moderate/severe chest indrawing, tachypnea ≥60 if <6 months.'),
  ...
  // ─── 22. Pneumonia peds severity ───────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('rrHigh', 'RR substantially above age normal (BTS: >70/min if infant, >50/min if older child)', 1,
        'BTS-style severe-CAP tachypnea: >70 breaths/min in infants, >50/min in older children. Alternatively, ~20–30 above PALS age-max RR (infant 30–53, toddler 22–37, preschool 20–28, school 18–25).'),
      ...
      yesNo('infant', 'Age <3 months (higher risk band)', 1,
        'IDSA/PIDS and BTS treat very young infants as higher-risk CAP. This item uses <3 months; some pathways extend concern through <6 months — apply local policy.'),
  ...
  // ─── 24. Malaria severity ──────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('impairedConscious', 'Impaired consciousness / coma (GCS <11 or Blantyre <3)', 1,
        'WHO: GCS <11 (or Blantyre coma score <3 in children).'),
      ...
      yesNo('acidosis', 'Acidosis / acidotic breathing (base deficit >8, HCO3 <15, or lactate ≥5)', 1,
        'WHO: base deficit >8 mEq/L, plasma bicarbonate <15 mmol/L, or venous lactate ≥5 mmol/L; or deep (Kussmaul) breathing.'),
      ...
      yesNo('anemia', 'Severe anemia (Hb <5 g/dL if <12 y; <7 g/dL if adult) with parasites', 1,
        'WHO: Hb <5 g/dL (child <12 y) or <7 g/dL (adult) with parasite density >10,000/µL.'),
      yesNo('renal', 'Acute kidney injury (Cr >3 mg/dL or urea >20 mmol/L) / oliguria', 1,
        'WHO: serum creatinine >3 mg/dL (265 µmol/L) or urea >20 mmol/L, or oliguria.'),
      yesNo('jaundice', 'Jaundice (bilirubin >3 mg/dL) with parasites >100,000/µL', 1,
        'WHO: plasma or serum bilirubin >3 mg/dL (50 µmol/L) with parasite density >100,000/µL.'),
      yesNo('pulmonary', 'Pulmonary edema / ARDS (radiographic edema or SpO2 <92% with RR >30)', 1,
        'WHO: radiographic pulmonary edema, or SpO2 <92% on room air with respiratory rate >30/min.'),
      yesNo('bleeding', 'Significant bleeding / DIC', 1),
      yesNo('shock', 'Shock (CRT ≥3 s or SBP <80 adult / <70 child)', 1,
        'WHO: compensated shock = capillary refill ≥3 s or temperature gradient; hypotensive shock = SBP <80 mmHg adult or <70 mmHg child.'),
      yesNo('hyperparasitemia', 'Hyperparasitemia (Pf >10%; CDC often >5% in non-immune travelers)', 1,
        'WHO: P. falciparum parasitemia >10%. CDC often uses >5% in non-immune travelers. Threshold varies by epidemiology.'),
    ],
    calculate(values) {
      const keys = [
        'impairedConscious',
  ...
  // ─── 25. Dengue warning signs ──────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('abdPain', 'Abdominal pain or tenderness', 1,
        'Persistent abdominal pain or tenderness, not a single brief cramp.'),
      yesNo('persistentVomiting', 'Persistent vomiting', 1,
        'Repeated vomiting that prevents adequate oral intake (often ≥3 episodes in 1 hour).'),
      yesNo('fluidAccum', 'Clinical fluid accumulation (ascites/effusion/edema)', 1,
        'Pleural effusion, ascites, or edema on exam or imaging — a WHO warning sign of plasma leakage.'),
      yesNo('mucosalBleed', 'Mucosal bleeding', 1,
        'Epistaxis, gum bleeding, hematemesis, melena, or menorrhagia — not isolated petechiae.'),
      yesNo('lethargy', 'Lethargy / restlessness', 1,
        'Change in alertness: lethargy or restlessness (WHO warning), not ordinary tiredness.'),
      yesNo('liver', 'Liver enlargement >2 cm', 1,
        'Liver edge >2 cm below the costal margin (WHO warning sign).'),
      yesNo('hctRising', 'Lab: rising hematocrit with rapid platelet fall', 1,
        'Hematocrit rising concurrent with a rapid drop in platelet count (plasma leakage).'),
      yesNo('shock', 'Shock (pulse pressure ≤20 mmHg, hypotension, or cold clammy skin / poor perfusion)', 3,
        'WHO compensated shock includes pulse pressure ≤20 mmHg; hypotensive shock is unrecordable/low BP with perfusion failure.'),
