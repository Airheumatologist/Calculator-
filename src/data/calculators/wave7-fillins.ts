...
export const wave7FillinsCalcs: Calculator[] = [
  // ─── 1. KFRE 8-variable ────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('age', 'Age', { unit: 'years', min: 18, max: 110, exampleValue: 65, helpText: 'Age in years at the time of the labs; the 8-variable KFRE uses age linearly, so the estimate rises steadily with each year.' }),
      selectInput('sex', 'Sex', [
        { label: 'Female', value: 'F' },
        { label: 'Male', value: 'M' },
      ], 'M', 'Sex enters the model as a coefficient (male carries the higher risk); use the registry value recorded in the chart.'),
      numberInput('egfr', 'eGFR', { unit: 'mL/min/1.73 m²', min: 5, max: 90, exampleValue: 45, helpText: 'eGFR in mL/min/1.73 m² from the same date as the urine ACR and labs. The model was derived in CKD with eGFR typically below 60.' }),
  ...
  // ─── 3. 4PEPS ──────────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'M', 'Male sex adds 2 points to the 4PEPS total; female is the reference.'),
      yesNo('crd', 'Chronic respiratory disease', -1, 'COPD, asthma, ILD, or other chronic lung disease. Scored −1 because it offers an alternate explanation for dyspnea — not a PE risk factor.', false),
      yesNo('hr80', 'Heart rate <80 /min', -1, 'HR <80 at assessment (protective / −1). Use the pulse you would record for pretest probability, not a later treated rate.', false),
      yesNo('chestDyspnea', 'Chest pain AND acute dyspnea', 1, 'Both chest pain and acute dyspnea must be present. Isolated chest pain or isolated dyspnea does not score.', true),
      yesNo('hormone', 'Hormonal estrogenic treatment', 2, 'Estrogen-containing OCP, HRT, or other estrogenic therapy at the time of assessment.', false),
      yesNo('priorVte', 'Personal history of VTE', 2, 'Personal history of VTE adds 2 points to the 4PEPS total; only a documented prior VTE counts, not a remote unconfirmed suspicion.', false),
      yesNo('syncope', 'Syncope', 2, 'Transient loss of consciousness with spontaneous recovery attributed to this presentation (not a remote faint).', false),
      yesNo('immobility', 'Immobility within the last 4 weeks (surgery, lower-limb plaster, or bedridden >3 days)', 2, 'Immobility within the last 4 weeks — surgery, lower-limb plaster cast, or bedridden for more than 3 days — adds 2 points.', false),
      yesNo('spo2', 'Pulse oxygen saturation <95%', 3, 'Pulse oxygen saturation below 95% adds 3 points, the largest single item; use the room-air reading where possible.', true),
      yesNo('calf', 'Calf pain and/or unilateral lower-limb edema', 3, 'Calf pain and/or asymmetric swelling/edema of one lower limb (4PEPS DVT-sign item).', false),
      yesNo('peLikely', 'PE is the most likely diagnosis', 5, 'Clinician gestalt: PE is more likely than the next competing diagnosis.', false),
    ],
    calculate(values) {
  ...
  // ─── 4. AIN risk (educational) ─────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('ppi', 'Current PPI', 2, 'Currently taking a proton-pump inhibitor (omeprazole, pantoprazole, etc.) — a leading contemporary AIN culprit.', true),
      yesNo('abx', 'Culprit antibiotic (e.g., beta-lactam, fluoroquinolone, sulfa)', 2, 'Exposure to a culprit antibiotic (beta-lactam, fluoroquinolone, sulfa, rifampin) adds 2 points; timing within the prior weeks matters more than the class alone.', false),
      yesNo('nsaid', 'NSAID exposure', 2, 'NSAID exposure adds 2 points. Include over-the-counter and combination products, which patients often do not report as medicines.', false),
      yesNo('pyuria', 'Sterile pyuria (WBCs on microscopy, no infection)', 2, 'WBCs on urine microscopy with negative culture (or no clinical UTI).', true),
      yesNo('wbcCasts', 'WBC casts', 3, 'White-cell casts on microscopy (not hyaline casts).', false),
      yesNo('rash', 'Drug rash', 1, 'Morbilliform or hypersensitivity rash attributed to the culprit drug (classic triad item — insensitive).', false),
      yesNo('fever', 'Fever attributed to drug hypersensitivity', 1, 'Fever not explained by the original infection once it is treated, attributed to drug hypersensitivity.', false),
      yesNo('eos', 'Peripheral eosinophilia (AEC ≥500/µL or ≥5%)', 1, 'Peripheral eosinophilia — absolute eosinophil count 500/µL or higher, or 5% or more — adds 1 point. Its absence does not exclude AIN, and urine eosinophils are unreliable.', false),
  ...
  // ─── 5. MAPS (educational HCM prognostic) ──────────────────────────────────
  {
    ...
    inputs: [
      numberInput('age', 'Age', { unit: 'years', min: 16, max: 90, exampleValue: 50, helpText: 'Age in years at assessment; younger age raises the relative weight of other sudden-death markers in HCM counseling.' }),
      ...
      ], 1, 'NYHA class at assessment (I–IV); class III–IV symptoms carry additional weight in the educational estimate.'),
      yesNo('syncope', 'Unexplained syncope', null, 'Unexplained TLOC judged not neurally mediated (not vasovagal). ESC treats recent syncope (often ≤6 months) as the high-impact marker.', false),
      yesNo('famScd', 'Family history of SCD attributed to HCM', null, 'SCD in ≥1 first-degree relative <40 years, or SCD at any age in a first-degree relative with documented HCM.', false),
      numberInput('lvh', 'Maximal LV wall thickness', { unit: 'mm', min: 13, max: 40, exampleValue: 20, helpText: 'Massive LVH typically ≥30 mm' }),
      yesNo('nsvt', 'NSVT on ambulatory ECG', null, '≥3 consecutive ventricular beats at ≥120 bpm lasting <30 s (ESC HCM Risk-SCD definition).', false),
      ...
      numberInput('la', 'Left atrial diameter (or equivalent LA size)', { unit: 'mm', min: 25, max: 70, exampleValue: 40, helpText: 'Left atrial diameter or equivalent LA size in mm from the most recent imaging; a larger atrium is an adverse HCM marker.' }),
      numberInput('gradient', 'Resting LVOT gradient', { unit: 'mmHg', min: 0, max: 150, exampleValue: 0, helpText: 'Resting LVOT gradient in mmHg from the last echo; gradients of 30 mmHg or more are considered obstructive.' }),
      yesNo('af', 'Atrial fibrillation (any)', null, 'Any history of AF (paroxysmal, persistent, or permanent), not only AF on this ECG.', false),
      yesNo('lge', 'Extensive late gadolinium enhancement', null, 'CMR; extensive/≥15% of LV mass is the usual high-risk descriptor', false),
    ],
    calculate(values) {
  ...
  // ─── 6. IMPROVE-DD ─────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('priorVte', 'Prior VTE', 3, 'Prior VTE adds 3 points — the heaviest item in the IMPROVE-DD model.', false),
      yesNo('thrombophilia', 'Known thrombophilia', 2, 'Congenital or acquired: e.g. factor V Leiden, prothrombin G20210A, protein C/S or antithrombin deficiency, antiphospholipid syndrome / lupus anticoagulant.', false),
      yesNo('paralysis', 'Current lower-limb paralysis', 2, 'Current lower-limb paralysis adds 2 points; include paraplegia from any cause, not only recent stroke.', false),
  ...
  // ─── 8. Revised Baux ───────────────────────────────────────────────────────
  {
    ...
    inputs: [
      numberInput('age', 'Age', { unit: 'years', min: 0, max: 110, exampleValue: 40, helpText: 'Age in years; rBaux adds age directly to the TBSA percentage, so the score roughly equals predicted mortality in the original data.' }),
      ...
      yesNo('inhalation', 'Inhalation injury', 17, 'Clinically diagnosed inhalation injury (closed-space fire, carbonaceous sputum, facial burns/singed hairs plus airway signs) or bronchoscopy-confirmed.', false),
    ],
    calculate(values) {
  ...
  // ─── 10. Full Ranson ───────────────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('age', 'Age >55 years', 1, 'These are the original (non-biliary) Ranson 1974 cutoffs. Gallstone pancreatitis uses slightly different age/WBC/glucose/LDH thresholds (Ranson 1982); this tool does not switch.', true),
      yesNo('wbc', 'WBC >16,000/µL', 1, 'Original non-biliary cutoff. Biliary variant uses >18,000/µL — this tool does not switch.', true),
      yesNo('glu', 'Glucose >200 mg/dL', 1, 'Original non-biliary cutoff. Biliary variant uses >220 mg/dL — this tool does not switch.', false),
      yesNo('ldh', 'LDH >350 U/L', 1, 'Original non-biliary cutoff. Biliary variant uses >400 U/L — this tool does not switch.', true),
      yesNo('ast', 'AST >250 U/L', 1, 'Admission AST above 250 U/L scores 1 of the 5 admission points.', false),
      yesNo('hct', 'Hematocrit drop >10% (48 h)', 1, 'A hematocrit fall greater than 10 percentage points between admission and 48 hours scores a 48-hour point; a rising hematocrit from hemoconcentration does not count here.', false),
      yesNo('bun', 'BUN rise >5 mg/dL (48 h)', 1, 'A BUN rise greater than 5 mg/dL over the first 48 hours scores; a falling BUN does not.', false),
      yesNo('ca', 'Calcium <8 mg/dL (48 h)', 1, 'Serum calcium below 8 mg/dL at 48 hours scores 1 point (correct for albumin if your lab reports ionized values).', false),
      yesNo('pao2', 'PaO₂ <60 mmHg (48 h)', 1, 'PaO₂ below 60 mmHg at 48 hours on room air scores 1 point.', false),
      yesNo('base', 'Base deficit >4 mEq/L (48 h)', 1, 'Base deficit greater than 4 mEq/L at 48 hours scores 1 point; use the arterial gas value.', false),
      yesNo('fluid', 'Fluid sequestration >6 L (48 h)', 1, 'Net fluid balance over 48 h (total intake − total output) >6 L — not gestalt “looks third-spaced.”', false),
    ],
    calculate(values) {
  ...
  // ─── 11. SAPS 3 (fuller new ID) ────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      ], 'none', 'Use the single highest-weighted condition only (do not add multiple). NYHA IV = symptoms at rest.'),
      ...
      ], 'ed', 'Intra-hospital location before ICU admission (OR, ED, another ICU, or ward) is a Box I/II characteristic that carries its own points before physiology is added.'),
      yesNo('vasoactive', 'Vasoactive drugs before ICU', null, 'Vasoactive drugs already running before ICU admission (not started after arrival).', false),
      ...
      ], 'unplanned', 'Planned versus unplanned ICU admission is scored as an admission-circumstance variable; unplanned admissions score higher.'),
      selectInput(
        'reason',
      ...
      ], 'none', 'Surgical status: scheduled surgery, no surgery, or emergency surgery — each carries different points, and emergency surgery is the highest.'),
      ...
      ], 'other', 'Anatomical site of surgery: transplant −11, trauma −8, CABG without valve −6, neurosurgery for CVA +5, no surgery/other 0. These negative points are part of the published model.'),
      ...
      ], 'resp', 'Acute infection at ICU admission: none, non-respiratory nosocomial, respiratory, or nosocomial respiratory — the last carries the most points.'),
      ...
      ], 'room', 'Use the worst oxygenation in the ±1 hour admission window only (not worst in 24 h).'),
    ],
    calculate(values) {
      const age = num(values.age, 65);
