...
export const wave7FillinsCalcs: Calculator[] = [
  // ─── 1. KFRE 8-variable ────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      numberInput('albumin', 'Serum albumin', { unit: 'g/dL', min: 1, max: 6, step: 0.1, defaultValue: 4.0 }),
      numberInput('phosphorus', 'Serum phosphorus', { unit: 'mg/dL', min: 1, max: 12, step: 0.1, defaultValue: 3.9 }),
      numberInput('bicarbonate', 'Serum bicarbonate', { unit: 'mEq/L', min: 8, max: 40, step: 0.1, defaultValue: 25 }),
      numberInput('calcium', 'Serum calcium', { unit: 'mg/dL', min: 5, max: 15, step: 0.1, defaultValue: 9.4 }),
    ],
    calculate(values) {
  ...
  // ─── 3. 4PEPS ──────────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('crd', 'Chronic respiratory disease', -1),
      yesNo('hr80', 'Heart rate <80 /min', -1),
      yesNo('chestDyspnea', 'Chest pain AND acute dyspnea', 1),
      yesNo('hormone', 'Hormonal estrogenic treatment', 2),
      ...
      yesNo('calf', 'Calf pain and/or unilateral lower-limb edema', 3),
      yesNo('peLikely', 'PE is the most likely diagnosis', 5),
    ],
    calculate(values) {
  ...
  // ─── 4. AIN risk (educational) ─────────────────────────────────────────────
  {
    ...
    inputs: [
      yesNo('ppi', 'Current PPI', 2),
      yesNo('abx', 'Culprit antibiotic (e.g., beta-lactam, fluoroquinolone, sulfa)', 2),
      yesNo('nsaid', 'NSAID exposure', 2),
      yesNo('pyuria', 'Sterile pyuria (WBCs on microscopy, no infection)', 2),
      yesNo('wbcCasts', 'WBC casts', 3),
      yesNo('rash', 'Drug rash', 1),
      yesNo('fever', 'Fever attributed to drug hypersensitivity', 1),
      yesNo('eos', 'Peripheral eosinophilia', 1),
    ...
    evidence: {
      ...
      validation:
        'Classic triad is present in a minority of biopsy-proven AIN. Moledina et al. developed an EHR model (NDT 2021) with different variables; this bedside score is educational only. Biopsy is gold standard.',
      references: [
        {
          ...
          citation: 'Moledina DG et al. Nephrol Dial Transplant. 2021',
          year: 2021,
          pmid: '34089711',
  ...
  // ─── 5. MAPS (educational HCM prognostic) ──────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('syncope', 'Unexplained syncope', null),
      yesNo('famScd', 'Family history of SCD attributed to HCM', null),
      numberInput('lvh', 'Maximal LV wall thickness', { unit: 'mm', min: 13, max: 40, defaultValue: 20, helpText: 'Massive LVH typically ≥30 mm' }),
      yesNo('nsvt', 'NSVT on ambulatory ECG', null),
      numberInput('ef', 'LVEF', { unit: '%', min: 15, max: 80, defaultValue: 65 }),
      ...
      yesNo('af', 'Atrial fibrillation (any)', null),
  ...
  // ─── 6. IMPROVE-DD ─────────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      yesNo('thrombophilia', 'Known thrombophilia', 2),
      yesNo('paralysis', 'Current lower-limb paralysis', 2),
      yesNo('cancer', 'Current cancer', 2),
      yesNo('immobility', 'Immobilization ≥7 days', 1),
      ...
      references: [
        {
          ...
          citation: 'Gibson CM et al. TH Open. 2017 / related IMPROVEDD reports',
          year: 2017,
          pmid: '31249943',
  ...
  // ─── 8. Revised Baux ───────────────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      numberInput('tbsa', 'TBSA burned', { unit: '%', min: 0, max: 100, defaultValue: 20 }),
      yesNo('inhalation', 'Inhalation injury', 17),
    ],
    calculate(values) {
    ...
    evidence: {
      ...
      references: [
        ...
        },
        {
          ...
          citation: 'Ranson JH. Am J Gastroenterol. 1982 / related Ranson criteria reports',
          year: 1974,
          pmid: '4275641',
        },
      ],
  ...
  // ─── 11. SAPS 3 (fuller new ID) ────────────────────────────────────────────
  {
    ...
    inputs: [
      ...
      selectInput('comorbid', 'Highest comorbidity', [
        ...
        { label: 'Cancer therapy (chemo/radio/steroids)', value: 'therapy' },
        { label: 'Hematologic cancer', value: 'heme' },
        { label: 'Chronic HF NYHA IV', value: 'hf' },
        { label: 'Cirrhosis', value: 'cirrhosis' },
        { label: 'AIDS', value: 'aids' },
        { label: 'Metastatic cancer', value: 'meta' },
      ]),
      numberInput('losBefore', 'Hospital LOS before ICU', { unit: 'days', min: 0, max: 90, defaultValue: 1 }),
      selectInput('location', 'Intra-hospital location before ICU', [
        { label: 'Operating room', value: 'or' },
        { label: 'Emergency room', value: 'ed' },
        { label: 'Other ICU', value: 'icu' },
        { label: 'Ward / other', value: 'ward' },
      ]),
      ...
      selectInput('reason', 'Primary reason for ICU admission', [
        { label: 'Other / not listed', value: 'other' },
        { label: 'Rhythm disturbance (−5)', value: 'rhythm' },
        { label: 'Seizures (−4)', value: 'seizure' },
        { label: 'Hypovolemic shock (+3)', value: 'hypovolemia' },
        { label: 'Acute abdomen / other digestive (+3)', value: 'abdomen' },
        { label: 'Coma / stupor / delirium (+4)', value: 'coma' },
        { label: 'Septic / anaphylactic / mixed shock (+5)', value: 'shock' },
        { label: 'Liver failure (+6)', value: 'liver' },
        { label: 'Focal neurologic deficit (+7)', value: 'focal' },
        { label: 'Severe pancreatitis (+9)', value: 'pancreatitis' },
        { label: 'Intracranial mass effect (+10)', value: 'mass' },
      ]),
      selectInput('surgical', 'Surgical status at ICU admission', [
        { label: 'Scheduled surgery', value: 'scheduled' },
        { label: 'No surgery', value: 'none' },
        { label: 'Emergency surgery', value: 'emergency' },
      ]),
      selectInput('anatSite', 'Anatomical site of surgery (if any)', [
        { label: 'No surgery / other', value: 'other' },
        { label: 'Transplant (−11)', value: 'transplant' },
        { label: 'Trauma (−8)', value: 'trauma' },
        { label: 'CABG without valve (−6)', value: 'cabg' },
        { label: 'Neurosurgery for CVA (+5)', value: 'cva' },
      ]),
      selectInput('infection', 'Acute infection at ICU admission', [
        { label: 'None', value: 'none' },
        { label: 'Nosocomial (not respiratory)', value: 'noso' },
        { label: 'Respiratory', value: 'resp' },
        { label: 'Nosocomial respiratory', value: 'both' },
      ]),
      numberInput('gcs', 'Lowest GCS (admission window)', { min: 3, max: 15, defaultValue: 15 }),
      numberInput('bili', 'Highest total bilirubin', { unit: 'mg/dL', min: 0, max: 40, step: 0.1, defaultValue: 0.8 }),
      numberInput('temp', 'Highest temperature', { unit: '°C', min: 30, max: 43, step: 0.1, defaultValue: 37 }),
      numberInput('creat', 'Highest creatinine', { unit: 'mg/dL', min: 0.1, max: 20, step: 0.1, defaultValue: 1.0 }),
      numberInput('hr', 'Highest heart rate', { unit: '/min', min: 30, max: 250, defaultValue: 90 }),
      numberInput('wbc', 'Leukocytes', { unit: '×10³/µL', min: 0, max: 100, step: 0.1, defaultValue: 10 }),
      numberInput('ph', 'Lowest pH', { min: 6.6, max: 7.7, step: 0.01, defaultValue: 7.38 }),
      numberInput('plt', 'Lowest platelets', { unit: '×10³/µL', min: 5, max: 800, defaultValue: 220 }),
      numberInput('sbp', 'Lowest systolic BP', { unit: 'mmHg', min: 30, max: 250, defaultValue: 120 }),
      selectInput('ox', 'Oxygenation (admission window)', [
        { label: 'PaO₂ ≥60 and not ventilated', value: 'room' },
        { label: 'PaO₂ <60, not ventilated', value: 'hypox' },
        { label: 'Ventilated, PaO₂/FiO₂ ≥100', value: 'ventOk' },
        { label: 'Ventilated, PaO₂/FiO₂ <100', value: 'ventLow' },
      ]),
    ],
    calculate(values) {
      const age = num(values.age, 65);
