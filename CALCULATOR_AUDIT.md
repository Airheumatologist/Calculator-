...
| 2 | Complete "Load example" coverage | **In progress — 685 of 1004 complete** (319 partial across 21 data files; measured 2026-09-16, second backfill pass). The original "overwhelmingly large questionnaires" characterisation was wrong: only 26 of the 674 unfinished calculators were questionnaires and 328 had no `exampleValue` at all, including plain scores (CHA₂DS₂-VASc, Wells, PERC, HEART, TIMI). `tests/registry-example-sweep.test.ts` now holds a `PENDING_EXAMPLE_BACKFILL` list: every file *outside* it must load a complete example, every complete example must survive the production pipeline without `NaN`/gate rejection/throw, and the sweep still asserts every remaining partial example is *gated* ("Enter all required inputs") rather than calculated from partial data. Worker tooling for the rest: `scratch-example/BRIEF.md` (rules + traps), `scratch-example/codemod.mjs` (positional `exampleValue` inserter, keyed per calculator so repeated input ids land correctly), `tests/tmp-example-worklists.test.ts` (`EXAMPLE_WRITE_WORKLISTS=1` → one worklist per module under `/tmp/example-worklists`), and `tests/tmp-example-scope.test.ts` (`EXAMPLE_AUDIT_FILES=…` → scoped coverage check). | Finish the remaining 21 files, then replace the ratchet assertion with `expect(offenders).toEqual([])` and delete `PENDING_EXAMPLE_BACKFILL` (plus the three scratch files above). Note for reviewers: `exampleValue` is per-required-input, must stay inside the declared `min`/`max` or option set, and `selectInput` values must match the option literal's type (number vs string) or `validateCalculator` rejects them; `yesNo`'s example is the **5th** argument, so appending to a four-argument call silently sets `required` instead. |
